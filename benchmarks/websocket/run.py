...
def main() -> int:
    ...
    parser.add_argument(
        "--label",
        default="",
        help="Human label recorded with this run in the unified cross-suite history.",
    )
    parser.add_argument(
        "--no-unified",
        action="store_true",
        help="Skip feeding the unified cross-suite dashboard (this suite's own reports only).",
    )
    parser.add_argument(
        "--expect-suites",
        default="",
        help=(
            "Comma-separated suites the unified record is SUPPOSED to end up with "
            "(e.g. 'http,websocket' when this run is one half of `make bench-all`). "
            "A record still missing one is then labeled incomplete rather than "
            "silently passing as the whole battery."
        ),
    )
    ...
    # Also feed the UNIFIED multi-suite record (benchmarks/out — the page with
    # the Suite selector where WebSocket sits next to HTTP), exactly as
    # benchmarks/http/run.py does for HTTP. The WebSocket-only report above
    # stays for deep WebSocket slicing; the unified page is where cross-subsystem
    # results live together. A record fed here carries the WebSocket suite ONLY —
    # `make bench-all` is what produces a record covering both suites.
    if not args.no_unified:
        # Flags gate the intent (--full), RESULTS gate the classification: a run
        # that asked for the full matrix but measured it with holes is a
        # DIAGNOSTIC too, quarantined out of every comparison surface.
        verdict = websocket_completeness(results, full_matrix=args.full)
        if not verdict.complete:
            print("\n=== RESTRICTED WEBSOCKET RUN — archiving as DIAGNOSTIC ===")
            for line in verdict.missing:
                print(f"  ✗ {line}")
            print(
                "  Kept for the investigation record under "
                f"{UNIFIED_OUTDIR}/diagnostics/, invisible to the unified "
                "dashboard and to merge targets. Re-run with --full for a record "
        ...
        fed = feed_unified(
            results,
            label=args.label,
            outdir=UNIFIED_OUTDIR,
            diagnostic=not verdict.complete,
            expected_suites=expect or None,
        )
        print(f"Unified record {fed.run_id} -> {UNIFIED_OUTDIR}/{fed.archive_dir}/")
        if fed.dashboard:
