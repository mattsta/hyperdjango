...
from benchmarks.http.connscaling import default_shard_count, run_conn_scaling
from benchmarks.http.contention import (
    GAUGE_ACTIVE,
    PEAK_GAUGES,
    ContentionSample,
    scrape,
...
# Set in main() from --cached-body. The JSON payload routes build a fresh body
# per request, so a payload cell measures the response path AND the per-request
# allocation/serialisation path together. The cached route serves byte-identical
# bodies from a module-level cache, isolating the response path alone — running
# a worker ladder both ways is what attributes a W-scaling drop to one or the
# other instead of guessing.
CACHED_BODY = False


def _payload_path(payload_bytes: int) -> str:
    if payload_bytes == 0:
        return "/plaintext"
    route = "/jsoncached" if CACHED_BODY else "/json"
    return f"{route}?n={payload_bytes}"


def _record_cell(cell: dict, payload_bytes: int, **fields) -> dict:
    """Attach a cell's identity fields AND its body byte rate.

    `body_gbps` (throughput x payload size) is recorded on EVERY row of every
    sweep, not just printed on the large-payload lines it happens to dominate:
    a number that only exists in a console column cannot be charted, compared
    across runs, or regression-gated. Small payloads legitimately record a
    near-zero rate — that IS the measurement, and it keeps the field's
    presence independent of the payload ladder a given run chose."""
    cell.update(payload_bytes=payload_bytes, **fields)
    cell["body_gbps"] = cell["throughput_rps"] * payload_bytes / 1e9
    return cell
...
def _select_payloads(args, default: list[tuple[str, int]]) -> list[tuple[str, int]]:
    """Apply --payloads (comma list of names) to a sweep's payload ladder.

    Names resolve against the FULL ladder (not the mode default), so a quick
    run can still target e.g. 64KiB. Unknown names fail loudly with the valid
    list — a typo must not silently run the whole ladder."""
    if not args.payloads:
        return default
    by_name = dict(PAYLOADS)
    chosen = []
    for name in args.payloads.split(","):
        name = name.strip()
        if not name:
            continue
        if name not in by_name:
            valid = ", ".join(n for n, _ in PAYLOADS)
            raise SystemExit(f"--payloads: unknown payload {name!r} (valid: {valid})")
...
def _bench_cell(
    ...
    def sampler():
        ...
        while not stop.is_set():
            # Memory at 1 Hz: PSS reads walk /proc/<pid>/smaps_rollup (~1 ms
            # per process), which at 10 Hz across a 64-worker tree would burn
            # most of a core. Peaks move slowly; 1 Hz loses nothing.
            if tick % 10 == 0:
            ...
            if sample_contention:
                ...
                if m:
                    ...
                    for g in PEAK_GAUGES:
                        v = m.get(g, 0.0)
                        gauge_peaks[g] = max(gauge_peaks.get(g, 0.0), v)
                        # Only count samples where the server has work in
                        # flight — the warmup/measure client handover leaves an
                        # idle instant whose "everything parked" reading would
                        # otherwise dominate the mean.
                        if m.get(GAUGE_ACTIVE, 0.0) > 0:
                            gauge_series.setdefault(g, []).append(v)
            time.sleep(0.1)
    ...
    if sample_contention:
        ...
        cont.peaks = gauge_peaks
        cont.series = gauge_series
...
def _concurrency_sweep(frameworks, args) -> None:
    payloads = _select_payloads(
        args, [("tiny", 0), ("1k", 1024)] if args.quick else PAYLOADS
    )
        ...
        try:
            with ServerFixture(fw, HOST, PORT, args.workers) as fx:
                for pname, pbytes in payloads:
                    path = _payload_path(pbytes)
                    for c in concurrencies:
                        cell = _bench_cell(
                            fx, path, c, args.duration, args.warmup, args.client_procs
                        )
                        _record_cell(cell, pbytes, framework=fw, payload=pname)
                        results.append(cell)
                        print(
...
def _worker_sweep(frameworks, args):
    ...
    payloads = _select_payloads(
        args, [("plaintext", 0), ("16KiB", 16384)] if args.quick else PAYLOADS
    )
    meta = {
        "frameworks": frameworks,
        "payloads": payloads,
                ...
                try:
                    with ServerFixture(
                        ...
                        for pname, pbytes in payloads:
                            ...
                            # The recorded body byte rate is echoed on the
                            # payloads where it is the thing that actually
                            # saturates. At large bodies a W-ladder stops
                            # measuring server capacity once this plateaus — it
                            # is then reporting the box's memory / loopback
                            # (and, on a split-NUMA pin, cross-socket)
                            # bandwidth, and no server-side change moves it.
                            gbs = ""
                            if pbytes >= 16384:
                                gbs = f"  {cell['body_gbps']:.2f} GB/s"
                            ...
                            print(
                                f"  {pname:5s} {cell['throughput_rps']:>10,.0f} rps{gbs}  "
...
def _conn_scaling_sweep(frameworks, args):
    ...
    bound = args.cs_p99_bound_ms
    shards = args.cs_client_procs or default_shard_count(args.client_cores)
    meta = {
        "frameworks": frameworks,
        "conns": conns,
        ...
        "path": "/plaintext",
        # The capacity headline's rule, recorded WITH the data: a max_held read
        # off a run whose bound is unknown is not a comparable number.
        "cs_p99_bound_ms": bound,
        "cs_client_procs": shards,
        **_config_meta(frameworks),
    }
    results: list[dict] = []
    for fw in frameworks:
        ...
        try:
            with ServerFixture(fw, HOST, PORT, w):
                for n in conns:
                    ...
                    # The conn-scaling sweep drives /plaintext only (0-byte
                    # JSON payload), so its recorded rate is 0 GB/s — the field
                    # is still written so every row in every sweep carries it.
                    _record_cell(r, 0, framework=fw, conns=n, payload="plaintext")
                    results.append(r)
                    print(
    ...
    # Auto-verdict: the regime's headline is CAPACITY (max_held), not peak rps —
    # plus the client-ceiling annotation on any series whose rps went flat while
    # still serving every connection inside the p99 bound.
    caps = conn_scaling_capacity(results, meta, bound)
    verdict = conn_scaling_verdict(caps, bound)
    meta["capacity_flags"] = verdict
    if verdict:
        print(f"\n=== CONNECTION CAPACITY (served >= 99%, p99 <= {bound:.0f}ms) ===")
        for line in verdict:
            print(f"  {line}")
    out = write_conn_scaling_reports(results, meta, args.outdir)
    print(f"\n[conn-scaling] report -> {out}")
    return results, meta
...
def main() -> int:
    ...
    ap.add_argument(
        "--payloads",
        default="",
        help="comma list of payload names (e.g. 'plaintext,64KiB') restricting "
        "the worker/concurrency sweeps — for targeted investigation of one "
        "payload regime; empty = the mode's default ladder",
    )
    ap.add_argument(
        "--cached-body",
        action="store_true",
        help="drive the JSON payloads against /jsoncached (body built once) "
        "instead of /json (body built per request). Byte-identical responses, "
        "so an A/B across a worker ladder attributes a scaling drop to the "
        "per-request allocation path vs the response/socket path",
    )
    ap.add_argument(
        "--sweep-concurrency",
        type=int,
    ...
    ap.add_argument(
        "--cs-p99-bound-ms",
        type=float,
        default=CS_P99_BOUND_MS,
        help="p99 bound (ms) defining a HELD connection in the conn-scaling "
        "capacity headline: max_held is the largest N served >=99%% inside it",
    )
    ap.add_argument(
        "--cs-client-procs",
        type=int,
        default=None,
        help="shard the conn-scaling think-time client across this many "
        "processes (default: one per 8 client cores, floor 2) so the load "
        "generator is not the ceiling; 1 = single-process (no IPC)",
    )
    ...
    ap.add_argument(
        "--check-against",
        default="",
        help="regression gate: after archiving, compare this run's per-series "
        "PEAK throughput against the most recent archived run with the given "
        "label; exit 3 if any series regresses beyond --check-tolerance",
    )
    ap.add_argument(
        "--check-tolerance",
        type=float,
        default=0.15,
        help="allowed fractional peak regression before the gate fails "
        "(default 0.15 — single-run peaks are ~±10%% noisy)",
    )
    ap.add_argument(
        "--expect-suites",
        default="",
        help="comma-separated suites the UNIFIED cross-suite record is supposed "
        "to end up with (e.g. 'http,websocket' when this run is one half of "
        "`make bench-all`); a record still missing one is then labeled "
        "incomplete instead of silently passing as the whole battery",
    )
    ap.add_argument("--outdir", default="benchmarks/http/out")
    args = ap.parse_args()

    global CLIENT, CACHED_BODY
    CACHED_BODY = args.cached_body
    ...
    # Machine prep runs in EVERY mode, not just the suite. A targeted-mode
    # run (--mode concurrency to re-check four accused cells) that silently
    # measures on the schedutil governor is NOT comparable to a suite run
    # measured on `performance` — observed as a spurious -15..-30% on
    # partially-loaded cells that "reproduced" a regression which was
    # actually reduced clocks. The prep restores the governor on exit either
    # way, so there is no reason to skip it for small runs.
    ...
    if args.mode == "all":
        ...
        # The worker sweep must SATURATE its own ladder: at the old default
        # c=128, a W=64 server averages 2 connections per worker, so workers
        # idle-wake per request and the sweep measures wakeup latency, not
        # capacity (observed: reactor W=64 66k rps at c=128 vs 479-579k at
        # c=1024, with per-request latency doubling every time conns/worker
        # halved). Keep at least ~16 busy connections per worker at the top
        # of the ladder; an explicit --sweep-concurrency still wins.
        if args.sweep_concurrency == 128:  # argparse default, not user-chosen
            args.sweep_concurrency = max(1024, 16 * budget)
        print(
    ...
    # A RESULT compares everything against everything else: the full
    # framework × payload × regime matrix. Anything narrower is a DIAGNOSTIC
    # (fix validation, bisection, A/B probe) — archived for the investigation
    # record but structurally invisible to the comparison history, the
    # dashboards, and the gate's baseline lookup, so partial runs can never
    # masquerade as comparison results.
    #
    # FLAGS GATE THE INTENT; RESULTS GATE THE CLASSIFICATION. Asking for the
    # whole matrix is not the same as measuring it: a comparison framework
    # whose optional deps are missing is skipped mid-run, a server can fail to
    # boot at one worker count, a payload can die on an OOM — and each leaves
    # a flag-complete run with holes. So the archived RESULTS are verified
    # before anything is called complete.
    flags_complete = (
        args.mode == "all"
        and set(frameworks) == set(ALL_FRAMEWORKS)
        and not args.payloads
    )
    verdict = verify_completeness(
        {"sweeps": build_sweeps(conc, conc_meta, work, work_meta, extra_sweeps)},
        ALL_FRAMEWORKS,
        [name for name, _ in PAYLOADS],
    )
    is_complete = flags_complete and verdict.complete
    if flags_complete and not verdict.complete:
        print(
            "\n=== INCOMPLETE MATRIX — archiving as DIAGNOSTIC ===\n"
            "This run ASKED for the full matrix (--mode all, every framework, "
            "the full payload ladder) but did not MEASURE it. Missing cells:"
        )
        for line in verdict.missing:
            print(f"  ✗ {line}")
        print(
            "  A run with holes cannot be a comparison baseline: the gate would "
            "read the absent series as 'not measured' and treat the rest as a "
            "baseline nobody can reproduce. Fix the cause (usually the optional "
            "comparison deps — run via `make bench-http`, or "
            "`uv run --group benchmark-comparison hyper-bench ...`) and re-run."
        )
    run_id = save_run(
        args.outdir,
        conc,
        ...
        diagnostic=not is_complete,
    )
    archive_dir = "history" if is_complete else "diagnostics"
    kind = "COMPLETE comparison run" if is_complete else "DIAGNOSTIC (partial) run"
    print(f"\nArchived {kind} -> {args.outdir}/{archive_dir}/{run_id}.json")

    # Regression gate: compare the JUST-ARCHIVED file (not the in-memory rows)
    # against the newest prior run carrying the requested label, so both sides
    # of the comparison share the exact archived shape. A failed gate defers
    # its exit code until AFTER the dashboards render — a regressed run is
    # precisely the one whose report you want to look at.
    gate_rc = 0
    if args.check_against:
        current = json.loads(
            (Path(args.outdir) / archive_dir / f"{run_id}.json").read_text()
        )
        baseline = find_run_by_label(args.outdir, args.check_against, exclude_id=run_id)
        if baseline is None:
            print(
                f"[gate] no archived run labeled {args.check_against!r} — "
                "nothing to compare against (gate SKIPPED, not failed)"
            )
        else:
            lines, ok = compare_runs(current, baseline, args.check_tolerance)
            print(
                f"\n=== REGRESSION GATE vs {baseline['id']} "
                f"(label={args.check_against!r}, tolerance {args.check_tolerance:.0%}) ==="
            )
            for line in lines:
                print(line)
            if ok:
                ...
                print("[gate] FAILED — peak throughput regressed beyond tolerance")
                gate_rc = 3

    # Comparison surfaces render from COMPLETE runs only — a diagnostic run
    # ends here (archived + optionally gated, never presented as a result).
    if not is_complete:
        return gate_rc

    # Also feed the UNIFIED multi-suite dashboard (benchmarks/out/index.html —
    # the page with the Suite selector where HTTP sits next to WebSocket).
    # The HTTP-only report below remains for deep HTTP slicing; the unified
    # page is where cross-subsystem results live together.
    try:
        core_entry = {"sweeps": {}}
        if work is not None and work_meta:
            core_entry["sweeps"]["workers"] = {"meta": work_meta, "results": work}
        if conc is not None and conc_meta:
            core_entry["sweeps"]["concurrency"] = {"meta": conc_meta, "results": conc}
        if "bounded" in extra_sweeps:
            b_res, b_meta = extra_sweeps["bounded"]
            core_entry["sweeps"]["bounded"] = {"meta": b_meta, "results": b_res}
        if "connscaling" in extra_sweeps:
            cs_res, cs_meta = extra_sweeps["connscaling"]
            core_entry["conn_scaling"] = {"meta": cs_meta, "results": cs_res}
        if core_entry["sweeps"] or core_entry.get("conn_scaling"):
            expect = [s.strip() for s in args.expect_suites.split(",") if s.strip()]
            core_save_run(
                "benchmarks/out",
                {"http": build_http_suite(core_entry)},
                label=args.label,
                cores=os.cpu_count(),
                expected_suites=expect or None,
            )
            dash = write_dashboard("benchmarks/out")
            print(f"Unified dashboard (all suites) -> {dash}")
    except ModuleNotFoundError as exc:
        print(f"Unified dashboard skipped ({exc}) — see the re-render note below.")
