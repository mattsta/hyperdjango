...
H3Error cellToBBox(H3Index cell, BBox *out, bool coverChildren) {
    ...
    if (res == 0) {
        ...
        *out = RES0_BBOXES[baseCell];
        ...
        /* Get the cell's Vec3d directly so we can derive cos(centerLat)
         * from v.z without a redundant cos() call. v is on the unit
         * sphere, so v.z = sin(centerLat); cos(centerLat) = sqrt(1 - v.z²).
         * fsqrt on AArch64/x86 is ~10-15 cycles vs libm cos ~30-50, and
         * cellToBBox is called once per coarse-cell descent in the
         * polyfill iterator, so this saves real cycles. */
        Vec3d v;
        H3Error e = cellToVec3(cell, &v);
        if (e != E_SUCCESS) {
            return e;
        }
        double centerLat = approx_asin(v.z);
        double centerLng = approx_atan2(v.y, v.x);
        double cosLat = sqrt(1.0 - v.z * v.z);
        double edge = MAX_EDGE_LENGTH_RADS[res];
        /* lngRatio = 1/cos(lat). When the cell is near a pole, cos(lat)
         * approaches zero and lngRatio explodes; the +/- pole adjustment
         * below handles those cases by widening to the full longitude
         * range, so a finite (even huge) lngRatio is fine here. The
         * downstream scaleBBox + pole tests clamp the output. */
        double lngRatio = 1.0 / cosLat;
        out->north = centerLat + edge;
        out->south = centerLat - edge;
        out->east = centerLng + edge * lngRatio;
        out->west = centerLng - edge * lngRatio;
    }
...
static H3Index nextCell(H3Index cell) {
    ...
    while (true) {
        ...
        if (digit < INVALID_DIGIT - 1) {
            H3_SET_INDEX_DIGIT(
                cell, res,
                digit + ((_h3IsPentagon(parent) && digit == CENTER_DIGIT)
 ...
 * value
 */
static IterCellsPolygonCompact
_iterInitPolygonCompact(const GeoPolygon *polygon, int res, uint32_t flags) {
    IterCellsPolygonCompact iter = {// Initialize output properties. The first
                                    ...
                                    ._bboxes = NULL,
                                    ._started = false,
                                    /* Lookahead buffer state — used only by
                                     * the CONTAINMENT_CENTER fast path. */
                                    ._pendingCount = 0,
                                    ._pendingIdx = 0,
                                    ._walkCell = H3_NULL,
                                    ._lastYielded = H3_NULL,
                                    ._walkInited = false};
...
/**
 * Internal core of the polyfill state machine. Walks forward from
 * `iter->cell` until it reaches a cell that should be yielded, sets
 * `iter->cell` to that cell, and returns.
 *
 * When `skipCenterTest` is true (the buffered CONTAINMENT_CENTER fast
 * path), the per-cell `pointInsidePolygon` check at target resolution
 * is skipped — every target-res cell encountered is yielded as a
 * "candidate" for the wrapper to batch-test downstream. When false
 * (every other mode and every other call), behavior is identical to
 * the legacy single-cell iterator.
 */
static void iterStepPolygonCompactInner(IterCellsPolygonCompact *iter,
                                        bool skipCenterTest) {
    ...
    while (cell) {
        ...
        if (cellRes == iter->_res) {
            if (mode == CONTAINMENT_CENTER && skipCenterTest) {
                /* Buffered fast path: yield this cell as a candidate
                 * without running pointInsidePolygon. The wrapper will
                 * batch-test all candidates via pointInsidePolygonBatch
    ...
    //
    // When called from the buffered CONTAINMENT_CENTER wrapper
    // (skipCenterTest = true), we must NOT destroy the iterator yet —
    // the wrapper still holds candidate cells that need to be batch-
    // tested against iter->_polygon and iter->_bboxes. Just signal EOF
    // by clearing iter->cell; the wrapper handles destruction once it
    // has finished any final batch and drained pending.
    if (skipCenterTest) {
        iter->cell = H3_NULL;
    } else {
        iterDestroyPolygonCompact(iter);
 ...
 *
 * For CONTAINMENT_CENTER mode, this wraps the inner state machine with a
 * lookahead buffer (size H3_POLYFILL_LOOKAHEAD) so that consecutive
 * target-resolution candidates are gathered into SoA arrays and tested
 * against the polygon in one `pointInsidePolygonBatch` call — the same
 * SIMD batch path the legacy `polygonToCells` already uses. Other modes
 * call straight through to the inner sequential logic.
 *
 * @param  iter Iterator to increment
 */
void iterStepPolygonCompact(IterCellsPolygonCompact *iter) {
    /* Detect external mutation of iter->cell between step calls. The
     * legacy API contract (exercised by testPolyfillInternal's invalid-
     * cell error paths) lets callers reset the walk by writing
     * iter->cell directly. When that happens the lookahead buffer is
     * stale and the walk cursor needs to follow. */
    if (iter->_walkInited && iter->cell != iter->_lastYielded) {
        iter->_walkCell = iter->cell;
        iter->_pendingCount = 0;
        iter->_pendingIdx = 0;
    }

    /* Drain the lookahead buffer first. Pending entries are cells already
     * proven to be yielded (either CENTER-mode candidates that passed the
     * batched PIP test, or coarse-resolution cells that the inner walker
     * already verified via cellBoundaryInsidePolygon). */
    if (iter->_pendingIdx < iter->_pendingCount) {
        iter->cell = iter->_pending[iter->_pendingIdx++];
        iter->_lastYielded = iter->cell;
        return;
    }

    if (FLAG_GET_CONTAINMENT_MODE(iter->_flags) != CONTAINMENT_CENTER) {
        /* Other modes use the original sequential path unchanged. */
        iterStepPolygonCompactInner(iter, false);
        iter->_lastYielded = iter->cell;
        return;
    }

    /* CONTAINMENT_CENTER fast path: refill the buffer.
     *
     * Iterator-state protocol:
     *   - `iter->_walkCell` is the cursor for the inner walker. The
     *     wrapper restores `iter->cell` from this before each inner call
     *     and saves it back afterward, so the public `iter->cell`
     *     (which gets set to pending entries during drain) doesn't
     *     pollute the walk position.
     *   - The first refill establishes _walkCell from the iterator's
     *     initial cell value (typically baseCellNumToCell(0)). */
    if (!iter->_walkInited) {
        iter->_walkCell = iter->cell;
        iter->_walkInited = true;
    }

    /* Loop until we have something to yield (pending non-empty) or we hit
     * EOF. Iteration is bounded by the polygon's cell coverage; each
     * inner call advances the walk cursor by at least one cell. */
    while (1) {
        H3Index candidates[H3_POLYFILL_LOOKAHEAD];
        double cLat[H3_POLYFILL_LOOKAHEAD];
        double cLng[H3_POLYFILL_LOOKAHEAD];
        int nCand = 0;
        H3Index coarseYield = H3_NULL;

        while (nCand < H3_POLYFILL_LOOKAHEAD) {
            /* Restore walk cursor before each inner call. */
            iter->cell = iter->_walkCell;
            iterStepPolygonCompactInner(iter, true);
            if (iter->error != E_SUCCESS) {
                return;
            }
            if (iter->cell == H3_NULL) {
                /* Inner reached EOF. iter->_polygon, iter->_bboxes etc
                 * were already cleared by iterDestroyPolygonCompact
                 * inside inner. */
                iter->_walkCell = H3_NULL;
                break;
            }
            /* Save new walk position. */
            iter->_walkCell = iter->cell;

            int r = H3_GET_RESOLUTION(iter->cell);
            if (r == iter->_res) {
                /* Target-res candidate. Compute center for batched test. */
                LatLng c;
                H3Error err = H3_EXPORT(cellToLatLng)(iter->cell, &c);
                if (err != E_SUCCESS) {
                    iterErrorPolygonCompact(iter, err);
                    return;
                }
                candidates[nCand] = iter->cell;
                cLat[nCand] = c.lat;
                cLng[nCand] = c.lng;
                nCand++;
            } else {
                /* Coarse-res cell already verified inside the polygon by
                 * the inner walker (cellBoundaryInsidePolygon path). It
                 * must be yielded after any pending candidates that came
                 * before it in iteration order. */
                coarseYield = iter->cell;
                break;
            }
        }

        /* Run the batched point-in-polygon test on accumulated
         * candidates. */
        iter->_pendingCount = 0;
        iter->_pendingIdx = 0;
        if (nCand > 0) {
            uint8_t inside[H3_POLYFILL_LOOKAHEAD];
            pointInsidePolygonBatch(iter->_polygon, iter->_bboxes,
                                    (size_t)nCand, cLat, cLng, inside);
            for (int i = 0; i < nCand; i++) {
                if (inside[i]) {
                    iter->_pending[iter->_pendingCount++] = candidates[i];
                }
            }
        }
        if (coarseYield != H3_NULL) {
            iter->_pending[iter->_pendingCount++] = coarseYield;
        }

        if (iter->_pendingCount > 0) {
            /* Have something to yield. */
            iter->cell = iter->_pending[iter->_pendingIdx++];
            iter->_lastYielded = iter->cell;
            return;
        }
        if (iter->_walkCell == H3_NULL) {
            /* Walk hit EOF and no candidates passed — terminate.
             * The inner walker deferred destroy back to us when in
             * skipCenterTest mode, so do it here. */
            iterDestroyPolygonCompact(iter);
            iter->_lastYielded = H3_NULL;
            return;
        }
        /* Otherwise, this batch had no matches and the walk is still
         * live. Loop and refill again. */
    }
}
...
H3Error H3_EXPORT(maxPolygonToCellsSizeExperimental)(const GeoPolygon *polygon,
    ...
    double polygonBBoxAreaKm2 =
        ...
        approx_cos(fmin(fabs(polygonBBox->north), fabs(polygonBBox->south))) *
