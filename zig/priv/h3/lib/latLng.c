 ...
 */
/* `normalizeLng` is now `static inline` in latLng.h — see the comment
 * there. */
...
double H3_EXPORT(greatCircleDistanceRads)(const LatLng *a, const LatLng *b) {
    double sinHalfLat, cosHalfLat_unused, sinHalfLng, cosHalfLng_unused;
    double sinLatA, cosLatA, sinLatB, cosLatB;
    approx_sincos((b->lat - a->lat) * 0.5, &sinHalfLat, &cosHalfLat_unused);
    approx_sincos((b->lng - a->lng) * 0.5, &sinHalfLng, &cosHalfLng_unused);
    approx_sincos(a->lat, &sinLatA, &cosLatA);
    approx_sincos(b->lat, &sinLatB, &cosLatB);

    double A =
        sinHalfLat * sinHalfLat + cosLatA * cosLatB * sinHalfLng * sinHalfLng;

    return 2 * approx_atan2(sqrt(A), sqrt(1 - A));
}
