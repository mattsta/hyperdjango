...
/* gridDisk interior offsets-table fast-path RETIRED with measured
 * evidence. Investigation: the H3 lattice is translation-invariant
 * within a face, so ring-k offsets in normalized ijk are constant.
 * Applying them to interior origins should produce target cells in
 * O(1) lattice-add per cell + a single _faceIjkToH3 encode, skipping
 * the per-step h3NeighborRotations digit walk.
 *
 * Implementation produced correct output (all 322 tests pass with
 * digit validation) but was 10-12× SLOWER than the existing scalar
 * walk on `gridDisk_classII_k{1,3,5,10}`. Root cause: `_faceIjkToH3`
 * does a full res-deep digit walk + base-cell lookup + rotations
 * per cell (~120 ops), while h3NeighborRotations in the sequential
 * walk averages ~3-5 digit iterations with early-return when nextDir
 * hits CENTER (~30-50 ops). The "more independent encodes" approach
 * traded sequential-cheap for independent-expensive and lost.
 *
 * Permanent baseline kept in benchmarkGridDiskCells.c as
 * `gridDisk_classII_k* / gridDisk_classII_k*_legacy` so any future
 * attempt to revive this approach will see the regression evidence. */
...
H3Error H3_EXPORT(gridDiskDistances)(H3Index origin, int k, H3Index *out,
                                     int *distances) {
    /* The offsets-table interior fast-path was tried but RETIRED with
     * measured evidence: even for the ideal case (Class II + non-
     * pentagon + interior origin), per-cell `_faceIjkToH3` (full
     * digit walk + base cell rotations) is 3-4× more expensive than
     * the sequential scalar walk's `h3NeighborRotations`, which
     * benefits from cache locality + early-return when nextDir hits
     * CENTER. Net effect on `gridDisk_classII_k1..k10`: 10-12×
     * SLOWER than the legacy scalar path. The offsets-table code +
     * `_gridDiskInteriorFastPath` stay in the file as a documented
     * dead-end; benchmark evidence is the legacy-baseline pair
     * `gridDisk_classII_k* / gridDisk_classII_k*_legacy` in
     * benchmarkGridDiskCells.c. */
...
H3Error H3_EXPORT(gridRing)(H3Index origin, int k, H3Index *out) {
    ...
    // Fast algo failed, fall back to slower, correct algo
    // and also wipe out array because contents untrustworthy.
    // The cast to size_t keeps the multiplication in size_t throughout —
    // `6 * k` in plain `int` arithmetic would overflow for very large k
    // (same shape as the historical `maxGridDiskSize` overflow fix in
    // 619a2a7f, where the int math was widened to int64_t). The caller
    // sized `out` via maxGridRingSize(), which already uses int64 math.
    memset(out, 0, (size_t)6 * (size_t)k * sizeof(H3Index));
...
H3Error h3NeighborRotations(H3Index origin, Direction dir, int *rotations,
    ...
    *rotations = *rotations % 6;
    if (*rotations > 0) {
        dir = _rotate60ccw_n_tbl[*rotations][dir];
    }
    ...
    if (_isBaseCellPentagon(newBaseCell)) {
        ...
        current = _h3RotatePent60ccw_n(current, newRotations);
        ...
        current = _h3Rotate60ccw_n(current, newRotations);
    }
...
Direction directionForNeighbor(H3Index origin, H3Index destination) {
    bool isPent = _h3IsPentagon(origin);
...
H3Error H3_EXPORT(polygonToCells)(const GeoPolygon *geoPolygon, int res,
    ...
    while (numSearchHexes > 0) {
        ...
        //
        // The inner loop is structured to batch the polygon point-in-poly
        // test across an entire ring (up to MAX_ONE_RING_SIZE candidates).
        // Pass 1: hash-dedup each ring cell against `out` and gather
        //   survivors with their cell-center lat/lng into SoA scratch arrays.
        // Pass 2: one call to pointInsidePolygonBatch tests all survivors
        //   against the polygon (outer loop + holes) using the dispatched
        //   SIMD batch kernel — amortizes the per-edge polygon traversal
        //   across all candidates in the ring.
        // Pass 3: re-probe each surviving slot (in case two candidates from
        //   the same ring hashed to the same slot) and write the inside
        //   cells to out[] / found[].
        ...
        while (currentSearchNum < numSearchHexes) {
            ...
            H3Index cand[MAX_ONE_RING_SIZE];
            double cLat[MAX_ONE_RING_SIZE];
            double cLng[MAX_ONE_RING_SIZE];
            int nCand = 0;
            for (int j = 0; j < MAX_ONE_RING_SIZE; j++) {
                ...
                cand[nCand] = hex;
                cLat[nCand] = hexCenter.lat;
                cLng[nCand] = hexCenter.lng;
                nCand++;
            }

            uint8_t inside[MAX_ONE_RING_SIZE];
            pointInsidePolygonBatch(geoPolygon, bboxes, (size_t)nCand, cLat,
                                    cLng, inside);
            ...
            for (int j = 0; j < nCand; j++) {
                ...
                if (out[loc] == hex) {
                    continue; // earlier batch sib hit
                }
                out[loc] = hex;
        ...
        search = found;
        found = temp;
        for (int64_t j = 0; j < numSearchHexes; j++) {
            found[j] = 0;
        }
