 ...
 */
int H3_EXPORT(getBaseCellNumber)(H3Index h) {
...
/* `_zeroIndexDigits` is now `static inline` in h3Index.h. */
...
H3Error H3_EXPORT(compactCells)(const H3Index *h3Set, H3Index *compactedSet,
    ...
    while (numRemainingHexes) {
        ...
        for (int64_t i = 0; i < numRemainingHexes; i++) {
            ...
            if (_h3IsPentagon(hashSetArray[i] & H3_RESERVED_MASK_NEGATIVE)) {
 ...
 */
int H3_EXPORT(isResClassIII)(H3Index h) {
    return H3_GET_RESOLUTION(h) % 2;
}
...
int H3_EXPORT(isPentagon)(H3Index h) {
    /* The public ABI stays; the body delegates to the `static inline`
     * `_h3IsPentagon` in h3Index.h so internal callers across the lib
     * can collapse the two-helper check without crossing a function-
     * call boundary, while external bindings (Python/Java/JS/etc) still
     * see this exported symbol. */
    return _h3IsPentagon(h);
}

/* `_h3LeadingNonZeroDigit` is now `static inline` in h3Index.h — see the
 * comment there for the constant-time clz-based scan. */

/* The four `_h3Rotate*` helpers are now `static inline` in h3Index.h —
...
H3Index _faceIjkToH3(const FaceIJK *fijk, int res) {
    ...
    if (_isBaseCellPentagon(baseCell)) {
        ...
        h = _h3RotatePent60ccw_n(h, numRots);
    } else {
        h = _h3Rotate60ccw_n(h, numRots);
    }
...
#define H3_LATLNGS_BATCH_STACK_THRESHOLD 64

/**
 * Bulk lat/lng → cell encoder. The lat/lng → Vec3d sin/cos pass is the
 * heavy stage; running it through h3SimdLatLngToVec3Batch reuses the
 * NEON 2-wide / AVX2 4-wide / AVX-512 8-wide approx_sincos kernels.
 * The downstream Vec3d → FaceIJK → H3Index pipeline is integer-heavy
 * (face dot products, ijk normalization, base-cell rotations) and stays
 * scalar per cell.
 */
H3Error H3_EXPORT(latLngsToCells)(const LatLng *latLngs, int64_t n, int res,
                                  H3Index *out) {
    if (res < 0 || res > MAX_H3_RES) {
        return E_RES_DOMAIN;
    }
    ...
    /* Small batch: SIMD setup overhead exceeds the gain. The scalar
     * latLngToCell already calls inlined approx_sincos so this is fast. */
    if (n < 8) {
        for (int64_t i = 0; i < n; i++) {
            H3Error e = H3_EXPORT(latLngToCell)(&latLngs[i], res, &out[i]);
            if (e) {
                return e;
            }
        }
    ...
    double stackLats[H3_LATLNGS_BATCH_STACK_THRESHOLD];
    double stackLngs[H3_LATLNGS_BATCH_STACK_THRESHOLD];
    double stackX[H3_LATLNGS_BATCH_STACK_THRESHOLD];
    double stackY[H3_LATLNGS_BATCH_STACK_THRESHOLD];
    double stackZ[H3_LATLNGS_BATCH_STACK_THRESHOLD];
    ...
    double *zs;
    int onHeap = 0;
    if (n <= H3_LATLNGS_BATCH_STACK_THRESHOLD) {
        lats = stackLats;
        lngs = stackLngs;
        xs = stackX;
        ys = stackY;
        ...
        lats = H3_MEMORY(malloc)((size_t)n * sizeof(double));
        lngs = H3_MEMORY(malloc)((size_t)n * sizeof(double));
        xs = H3_MEMORY(malloc)((size_t)n * sizeof(double));
        ys = H3_MEMORY(malloc)((size_t)n * sizeof(double));
        zs = H3_MEMORY(malloc)((size_t)n * sizeof(double));
        if (!lats || !lngs || !xs || !ys || !zs) {
            H3_MEMORY(free)(lats);
            H3_MEMORY(free)(lngs);
            H3_MEMORY(free)(xs);
            H3_MEMORY(free)(ys);
    ...
    /* AoS → SoA + lat/lng domain validation. */
    H3Error err = E_SUCCESS;
    for (int64_t i = 0; i < n; i++) {
        if (!isfinite(latLngs[i].lat) || !isfinite(latLngs[i].lng)) {
            err = E_LATLNG_DOMAIN;
            goto cleanup;
        }
        lats[i] = latLngs[i].lat;
        lngs[i] = latLngs[i].lng;
    }

    /* Stage 1: SIMD-batched lat/lng → Vec3d via the existing dispatch
     * (AVX-512 8-wide / AVX2 4-wide / scalar approx_sincos chain). */
    h3SimdLatLngToVec3Batch(lats, lngs, (size_t)n, xs, ys, zs);

    /* Stage 2: per-cell Vec3d → FaceIJK → H3Index. Scalar (integer-heavy
     * pipeline). With H3_ENABLE_OPENMP the loop parallelizes across cores
     * for large n; iteration writes to disjoint slots in out[]. */
    H3_OMP_PARALLEL_FOR_LARGE(n)
    for (int64_t i = 0; i < n; i++) {
        if (err) {
            continue;
        }
        Vec3d v = {.x = xs[i], .y = ys[i], .z = zs[i]};
        H3Error e = vec3ToCell(&v, res, &out[i]);
        if (e) {
            H3_OMP_ATOMIC_WRITE
            err = e;
        }
...
int _h3ToFaceIjkWithInitializedFijk(H3Index h, FaceIJK *fijk) {
    ...
    /* Fused (downAp7 ; neighbor) per iteration via
     * `_downAp7_neighbor` / `_downAp7r_neighbor` in coordijk.h. Each
     * iteration runs 3 multiplies + 6 additions + 1 normalize instead
     * of the original 9 mul + 7 adds + 2 normalize. */
    for (int r = 1; r <= res; r++) {
        Direction digit = H3_GET_INDEX_DIGIT(h, r);
        if (isResolutionClassIII(r)) {
            _downAp7_neighbor(ijk, digit); // Class III == rotate ccw
        } else {
            _downAp7r_neighbor(ijk, digit); // Class II == rotate cw
        }
    }
...
/* Res-0 cell-center lookup table.
 *
 * There are only NUM_BASE_CELLS = 122 valid res-0 cells; their lat/lng
 * centers are immutable. Computing them on-demand requires the full
 * `_h3ToFaceIjk → _faceIjkToVec3 → vec3ToLatLng` pipeline (~100 cycles).
 * Caching them in a 122-entry table reduces res-0 `cellToLatLng` to a
 * mode/res check + bounds check + table load (~10 cycles, ~10× faster).
 *
 * Lazy-init pattern with `__atomic_*` builtins (C99-compatible — works
 * across gcc/clang without `<stdatomic.h>` / C11):
 *   - First caller computes all 122 entries, then sets `ready` with
 *     release semantics.
 *   - Subsequent callers acquire-load `ready`; if non-zero, the array
 *     contents are guaranteed visible.
 *   - Concurrent first-callers each compute and write identical values;
 *     races are benign (writes are idempotent), and the final
 *     release-store guarantees visibility.
 *
 * This table is a perf optimization — correctness still falls back to
 * the full pipeline if the input fails the fast-path predicates. */
static LatLng res0_centers_cache[NUM_BASE_CELLS];
static int res0_centers_ready;

static void _initRes0Centers(void) {
    LatLng tmp[NUM_BASE_CELLS];
    for (int bc = 0; bc < NUM_BASE_CELLS; bc++) {
        H3Index h = H3_INIT;
        H3_SET_MODE(h, H3_CELL_MODE);
        H3_SET_BASE_CELL(h, bc);
        FaceIJK fijk;
        Vec3d v;
        if (_h3ToFaceIjk(h, &fijk) == E_SUCCESS) {
            _faceIjkToVec3(&fijk, 0, &v);
            tmp[bc] = vec3ToLatLng(v);
        } else {
            /* Should be unreachable — every base cell index 0..121 is valid
             * at res 0. Keep a sentinel so a buggy bypass doesn't return
             * uninitialized memory. */
            tmp[bc].lat = 0.0;
            tmp[bc].lng = 0.0;
        }
    }
    /* Bulk copy to the global cache, then publish the ready flag with
     * release semantics so any subsequent acquire-load on `ready` sees
     * the full array contents. */
    for (int bc = 0; bc < NUM_BASE_CELLS; bc++) {
        res0_centers_cache[bc] = tmp[bc];
    }
    __atomic_store_n(&res0_centers_ready, 1, __ATOMIC_RELEASE);
}
...
H3Error H3_EXPORT(cellToLatLng)(H3Index h3, LatLng *g) {
    /* Res-0 fast path: direct table lookup, skipping the full
     * `_h3ToFaceIjk → _faceIjkToVec3 → vec3ToLatLng` pipeline. We require
     * mode == H3_CELL_MODE and base cell in [0, NUM_BASE_CELLS) so the
     * fast path doesn't return wrong answers for non-cell indexes
     * (vertex/edge modes) or for indexes with reserved bits set
     * incorrectly. The slow path validates these via `_h3ToFaceIjk`. */
    if (H3_GET_RESOLUTION(h3) == 0 && H3_GET_MODE(h3) == H3_CELL_MODE &&
        H3_GET_RESERVED_BITS(h3) == 0) {
        int bc = H3_GET_BASE_CELL(h3);
        if ((unsigned)bc < (unsigned)NUM_BASE_CELLS) {
            if (!__atomic_load_n(&res0_centers_ready, __ATOMIC_ACQUIRE)) {
                _initRes0Centers();
            }
            *g = res0_centers_cache[bc];
            return E_SUCCESS;
        }
        /* Out-of-range base cell — fall through to the slow path so
         * `_h3ToFaceIjk` returns the proper error code. */
    }
    Vec3d v;
 ...
 */
/* Res-0 boundary lookup table — sibling to the res-0 centers cache
 * above. Stores all 122 base-cell boundaries (CellBoundary is at most
 * 10 vertices but typical hex cells use 6, pentagons 5; the array
 * holds the full struct so lookups are a single struct copy). Lazy-
 * init with the same release/acquire flag pattern as
 * `res0_centers_ready`.
 *
 * Memory cost: 122 × sizeof(CellBoundary) = 122 × ~176 bytes ≈ 21 KB
 * static. Trade-off vs. always running the full
 * `_h3ToFaceIjk → _faceIjk{Pent,}ToCellBoundary` pipeline (which
 * includes a 6-vertex SIMD trig batch + edge-crossing detection) —
 * worth it because res-0 boundaries are immutable and the slow path
 * is non-trivial. */
static CellBoundary res0_boundaries_cache[NUM_BASE_CELLS];
static int res0_boundaries_ready;

static void _initRes0Boundaries(void) {
    CellBoundary tmp[NUM_BASE_CELLS];
    for (int bc = 0; bc < NUM_BASE_CELLS; bc++) {
        H3Index h = H3_INIT;
        H3_SET_MODE(h, H3_CELL_MODE);
        H3_SET_BASE_CELL(h, bc);
        ...
        if (_h3ToFaceIjk(h, &fijk) == E_SUCCESS) {
            if (_h3IsPentagon(h)) {
                _faceIjkPentToCellBoundary(&fijk, 0, 0, NUM_PENT_VERTS,
                                           &tmp[bc]);
            } else {
                _faceIjkToCellBoundary(&fijk, 0, 0, NUM_HEX_VERTS, &tmp[bc]);
            }
        } else {
            tmp[bc].numVerts = 0;
        }
    ...
    for (int bc = 0; bc < NUM_BASE_CELLS; bc++) {
        res0_boundaries_cache[bc] = tmp[bc];
    }
    __atomic_store_n(&res0_boundaries_ready, 1, __ATOMIC_RELEASE);
}

H3Error H3_EXPORT(cellToBoundary)(H3Index h3, CellBoundary *cb) {
    /* Res-0 fast path — same predicate (mode/res/reserved-bits/bounds)
     * as the cellToLatLng res-0 cache above so non-cell or out-of-
     * range inputs fall through to the slow path's validation. */
    if (H3_GET_RESOLUTION(h3) == 0 && H3_GET_MODE(h3) == H3_CELL_MODE &&
        H3_GET_RESERVED_BITS(h3) == 0) {
        int bc = H3_GET_BASE_CELL(h3);
        if ((unsigned)bc < (unsigned)NUM_BASE_CELLS) {
            if (!__atomic_load_n(&res0_boundaries_ready, __ATOMIC_ACQUIRE)) {
                _initRes0Boundaries();
            }
            *cb = res0_boundaries_cache[bc];
            return E_SUCCESS;
        }
    }
    FaceIJK fijk;
...
/* `cellsToLatLngs` and `cellsToBoundaries` are defined in faceijk.c
 * because they need direct access to the static-inline SIMD helpers
 * `_hex2dToVec3_pair` / `_hex2dToVec3_x8_avx512` / `_vec3_to_latlng_*`
 * that live there. */
 ...
 */
int H3_EXPORT(pentagonCount)(void) {
 ...
 *         a Class II grid.
 */
/* `isResolutionClassIII` is now `static inline` in h3Index.h — see the
 * comment there. */
...
H3Error H3_EXPORT(cellToChildPos)(H3Index child, int parentRes, int64_t *out) {
    ...
    int parentIsPentagon = _h3IsPentagon(parent);
    ...
    *out = 0;
    if (parentIsPentagon) {
        ...
        for (int res = childRes; res > parentRes; res--) {
            ...
            parentIsPentagon = _h3IsPentagon(parent);
