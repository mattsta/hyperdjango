...
/* Stack-scratch threshold for the per-hole survivor mask. Matches the
 * pointInPoly kernels' threshold so the polygonToCells hot caller (which
 * passes n ≤ 7) never touches malloc here. */
#define H3_HOLE_MASK_STACK_SCRATCH 64

void pointInsidePolygonBatch(const GeoPolygon *geoPolygon, const BBox *bboxes,
                             size_t n, const double *lats, const double *lngs,
                             uint8_t *out) {
    if (n == 0) {
        return;
    }
    /* Outer-loop containment for all n points in one batch call. */
    h3SimdPointInsideGeoLoopBatch(geoPolygon->geoloop.numVerts,
                                  geoPolygon->geoloop.verts, &bboxes[0], n,
                                  lats, lngs, out);
    if (geoPolygon->numHoles == 0) {
        return;
    }

    /* For polygons with holes, batch each hole against ALL n points and OR
     * the result into out[]. Stack scratch for small n; heap fallback for
     * large batches; per-point fallback if alloc fails (preserves
     * correctness without crashing). */
    uint8_t stackHole[H3_HOLE_MASK_STACK_SCRATCH];
    uint8_t *hole;
    if (n <= H3_HOLE_MASK_STACK_SCRATCH) {
        hole = stackHole;
    } else {
        hole = malloc(n);
        if (!hole) {
            for (size_t i = 0; i < n; i++) {
                ...
                LatLng p = {.lat = lats[i], .lng = lngs[i]};
                for (int h = 0; h < geoPolygon->numHoles; h++) {
                    if (pointInsideGeoLoop(&geoPolygon->holes[h],
                                           &bboxes[h + 1], &p)) {
                        out[i] = 0;
                        break;
                    }
    ...
    for (int h = 0; h < geoPolygon->numHoles; h++) {
        h3SimdPointInsideGeoLoopBatch(geoPolygon->holes[h].numVerts,
                                      geoPolygon->holes[h].verts,
                                      &bboxes[h + 1], n, lats, lngs, hole);
        for (size_t i = 0; i < n; i++) {
            if (hole[i]) {
                out[i] = 0;
            }
