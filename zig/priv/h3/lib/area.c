...
/* Cagnoli contribution for edge arc x to y, following d3-geo's area
 * implementation:
 * https://github.com/d3/d3-geo/blob/8c53a90ae70c94bace73ecb02f2c792c649c86ba/src/area.js#L51-L70
 *
 * The earlier batched-cagnoli rewrite (lat-sincos pre-pass into SoA
 * arrays, then per-edge consumption) regressed on small n: the per-
 * call setup cost — stack-array reserve, stack-vs-heap if-check, two
 * pointer init, pre-pass-then-edge-loop dispatch — exceeded the
 * savings from caching 6 vertex lat-sincos values. The inline form
 * below lets the compiler fully fuse cagnoli into the per-edge loop.
 * See `cellAreaRads2_legacy` in benchmarkArea.c for the persistent
 * baseline that catches any regression of this trade-off. */
...
H3Error geoLoopAreaRads2(GeoLoop loop, double *out) {
    /* Use `Adder` to improve numerical accuracy of the sum of many
     * Cagnoli terms. */
    Adder adder = {};
    ...
    /* The Cagnoli sum yields a signed area; normalize negative
     * orientations into [0, 4π] by adding 4π. */
    if (adder.sum < 0.0) {
        kadd(&adder, 4.0 * M_PI);
    }
    *out = adder.sum;
