...
static inline H3Error cellToEdgeArcs(H3Index h, Arc *arcs,
    ...
    for (int64_t i = 0; i < numEdges; i++) {
        ...
        // The `arcs` parameter points into a sum-sized buffer
        // (`arcset->arcs` of size `getNumEdges`); each call writes the
        // slice `[0, numEdges)`. clang-analyzer can't see the
        // sum/slice relationship and warns ArrayBound. Verified by
        // hand: cur ∈ idx[0..numEdges-1] which for both idxh ({0,4,3,5,1,2})
        // and idxp ({0,1,3,2,4}) is a permutation of [0, numEdges).
...
static H3Error createSortableLoopSet(ArcSet arcset, SortableLoopSet *loopset) {
    ...
    // `numLoops > 0` is guaranteed by the caller chain: cellsToMultiPolygon
    // returns early on numCells == 0 and validateCellSet rejects duplicates,
    // so every non-empty validated cell set has a non-empty topological
    // boundary => at least one loop in the arcset. The static analyzer
    // can't reason about that topological invariant and warns malloc(0).
...
static H3Error createSortablePoly(SortableLoop *sloop, int64_t numHoles,
    ...
    // `sloop` points into a SortableLoopSet slice owned by createMultiPolygon,
    // which only calls us with `j > i` (slice spans at least one loop). The
    // analyzer doesn't propagate that across the call boundary and warns
    // sloop[0] could be OOB. Verified by inspection at call site (line 545).
    // NOLINTNEXTLINE(clang-analyzer-security.ArrayBound)
...
static H3Error createMultiPolygon(SortableLoopSet loopset,
    ...
    // `numPolys >= 1` is guaranteed: createMultiPolygon already returned
    // early at line 526 if `loopset.numLoops == 0`, and countPolys counts
    // distinct `root` values (which start at H3_NULL and increment on
    // every distinct sloops[i].root != cur). With numLoops > 0 the first
    // iteration always increments numPolys to >= 1. The analyzer doesn't
    // see the cross-call invariant and warns malloc(0).
    // NOLINTNEXTLINE(clang-analyzer-optin.portability.UnixAPI)
