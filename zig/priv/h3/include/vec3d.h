 ...
 *  Uses approx_sincos (Cephes double-precision polynomials, ≤180 nm
 *  absolute error vs. libm — see approx_libm.h) instead of libm sin/cos
 *  pairs. The H3 precision budget is half a meter, so the polynomial
 *  drift is invisible at any resolution. */
static inline Vec3d latLngToVec3(LatLng geo) {
    double sLat, cLat, sLng, cLng;
    approx_sincos(geo.lat, &sLat, &cLat);
    approx_sincos(geo.lng, &sLng, &cLng);
    Vec3d out = {
        .x = cLng * cLat,
        .y = sLng * cLat,
        .z = sLat,
...
static inline double vec3NormSq(Vec3d v) {
    return vec3Dot(v, v);
}

static inline double vec3Norm(Vec3d v) {
    return sqrt(vec3NormSq(v));
}
