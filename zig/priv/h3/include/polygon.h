...
bool pointInsidePolygon(const GeoPolygon *geoPolygon, const BBox *bboxes,
                        const LatLng *coord);
/**
 * Batch variant of pointInsidePolygon. Tests n points against the same
 * polygon (outer loop minus any holes) using the dispatched SIMD batch
 * kernel. SoA inputs (separate lat/lng arrays); each output byte is 1 if
 * the corresponding point is inside, 0 otherwise.
 *
 * Equivalent to calling pointInsidePolygon() per point, modulo the same
 * documented FP ordering caveats as h3SimdPointInsideGeoLoopBatch.
 */
