...
/* Lookahead buffer size for the experimental polyfill iterator's
 * CONTAINMENT_CENTER fast path. The iterator gathers up to this many
 * consecutive target-resolution candidates from its walk, then runs
 * `pointInsidePolygonBatch` once over all centers in lockstep — the same
 * batch path the legacy `polygonToCells` already uses. 8 is a sweet spot:
 * large enough to amortize per-edge polygon traversal across the batch,
 * small enough to fit on the stack with no allocator pressure. */
#define H3_POLYFILL_LOOKAHEAD 8
...
typedef struct {
    ...
    /* Lookahead buffer for the CONTAINMENT_CENTER fast path. Cells whose
     * centers have already been verified inside the polygon (via the SIMD
     * batch test) are queued here and yielded one at a time on subsequent
     * iterStepPolygonCompact calls. Other containment modes leave these
     * fields unused and fall through the original sequential code path. */
    H3Index _pending[H3_POLYFILL_LOOKAHEAD + 1];
    int _pendingCount;
    int _pendingIdx;
    H3Index _walkCell;    /* cell from which the next walk resumes */
    H3Index _lastYielded; /* iter->cell value at our last return — lets us
                             detect user mutation of iter->cell between
                             calls (legacy API contract; see
                             testPolyfillInternal invalid-cell paths) */
    bool _walkInited;     /* false until first walk has populated _walkCell */
