...
/** Maximum sum of (i + j + k) for a Class-II cell at each resolution to
 *  remain on its home face. Class III resolutions store -1 (use
 *  substrate at res+1). Exposed so the gridDisk interior fast-path
 *  in algos.c can decide if a ring of cells stays on a single face. */
extern const int maxDimByCIIres[];
