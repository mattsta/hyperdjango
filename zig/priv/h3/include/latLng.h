...
/* `normalizeLng` is `static inline` here because it's called four times
 * per `bboxOverlapsBBox` / `bboxContainsBBox` (now also inline) — the
 * function-call overhead used to dominate the actual switch+add work.
 * The const-evaluable switch arms collapse at every call site once the
 * normalization constant is known to the optimizer. */
static inline double normalizeLng(const double lng,
