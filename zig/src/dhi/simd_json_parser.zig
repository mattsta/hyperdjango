...
pub fn parseJsonObject(
    ...
    // Multi-word "field seen" bitset. A single u64 silently dropped every field
    // ≥64, and `@as(u64, 1) << @intCast(i)` for i ≥ 64 is an illegal @intCast
    // (UB in ReleaseFast, panic in ReleaseSafe). One bit per field across []u64
    // keeps the shift amount word-relative (0..63) and tracks all fields.
    const seen_words = try allocator.alloc(u64, @max(1, (n_fields + 63) / 64));
    @memset(seen_words, 0);
        ...
        for (field_specs, 0..) |spec, i| {
            if (spec.name_hash == field_hash) {
                ...
                if (std.mem.eql(u8, field_name, spec_name)) {
                    ...
                    seen_words[i >> 6] |= (@as(u64, 1) << @intCast(i & 63));
    ...
    for (field_specs, 0..) |spec, i| {
        if (spec.required != 0 and (seen_words[i >> 6] & (@as(u64, 1) << @intCast(i & 63))) == 0) {
            allocator.free(values);
    ...
    while (i < input.len) {
        if (input[i] == '\\' and i + 1 < input.len) {
            ...
            switch (next) {
                ...
                'u' => {
                    // Unicode escape: \uXXXX (with surrogate pair support for non-BMP)
                    if (i + 6 > input.len) {
                    ...
                    var codepoint: u21 = std.fmt.parseInt(u21, hex, 16) catch {
                        try result.append(allocator, input[i]);
                        i += 1;
                        continue;
                    };
                    ...
                    // RFC 8259: surrogate pairs \uD800-\uDBFF + \uDC00-\uDFFF
                    if (codepoint >= 0xD800 and codepoint <= 0xDBFF) {
                        // High surrogate — must be followed by \uDCxx low surrogate
                        if (i + 6 <= input.len and input[i] == '\\' and input[i + 1] == 'u') {
                            const low_hex = input[i + 2 .. i + 6];
                            const low = std.fmt.parseInt(u21, low_hex, 16) catch 0;
                            if (low >= 0xDC00 and low <= 0xDFFF) {
                                // Combine: (high - 0xD800) * 0x400 + (low - 0xDC00) + 0x10000
                                codepoint = @as(u21, @intCast(
                                    (@as(u32, codepoint) - 0xD800) * 0x400 +
                                        (@as(u32, low) - 0xDC00) + 0x10000,
                                ));
                                i += 6; // consume the \uXXXX low surrogate
                            }
                        }
                    }
                    ...
                    const len = std.unicode.utf8Encode(codepoint, &buf) catch {
                        // Still invalid (lone surrogate) — skip
                        continue;
                    };
...
test "parseJsonObject: tracks required fields beyond index 63" {
    const allocator = std.testing.allocator;
    const n = 70;

    // Stable name storage: "f0".."f69". field 65 (> u64 range) is required.
    var name_bufs: [n][8]u8 = undefined;
    var specs: [n]CFieldSpec = undefined;
    for (0..n) |i| {
        const name = std.fmt.bufPrint(&name_bufs[i], "f{d}", .{i}) catch unreachable;
        specs[i] = std.mem.zeroes(CFieldSpec);
        specs[i].name_ptr = name.ptr;
        specs[i].name_len = name.len;
        specs[i].name_hash = hashFieldName(name);
        specs[i].type_code = @intFromEnum(TypeCode.int);
        specs[i].required = if (i == 65) 1 else 0;
    }

    // Providing the required field ≥64 → seen, no MissingRequiredField.
    // (Pre-fix, setting/checking bit 65 was illegal-cast UB / a ReleaseSafe panic.)
    {
        const result = try parseJsonObject("{\"f65\": 123}", &specs, allocator);
        defer allocator.free(result.values);
        try std.testing.expect(result.values[65] != null);
        try std.testing.expectEqual(@as(i64, 123), result.values[65].?.int);
    }

    // Omitting the required field ≥64 → MissingRequiredField (bit is truly checked).
    {
        try std.testing.expectError(error.MissingRequiredField, parseJsonObject("{\"f0\": 1}", &specs, allocator));
    }
}
