// Exposed within this library — incorporated into hyperdjango
const std = @import("std");

// OpenSSL disabled — hyperdjango uses local PostgreSQL connections
...
pub const has_openssl = false;
pub const SSLCtx = void;
pub const default_column_names = true;
...
pub const TypeError = error{
    ...
    UnexpectedNull,
    UnknownColumnName,
    // A wire value whose inner length prefix / payload is inconsistent with the
    // bytes actually present (truncated, oversized, or negative-length field).
    // Raised by decoders that validate inner field lengths against the payload.
    InvalidData,
};
