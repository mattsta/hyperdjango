...
// Declared directly (not via std.c) so the symbol resolves on every libc we
// target; std.c compiles it out for glibc < 2.28-era version floors.
...
const SASL = struct {
    ...
    pub fn init(allocator: Allocator) !SASL {
        ...
        // libc getentropy, not arc4random_buf: arc4random_buf only exists in
        // glibc >= 2.36, so referencing it breaks builds targeting older
        // Linux. getentropy is everywhere we build (macOS 10.12+, glibc
        // 2.25+, musl) and cannot short-read below its 256-byte cap.
        if (getentropy(&nonce, nonce.len) != 0) return error.EntropyUnavailable;
