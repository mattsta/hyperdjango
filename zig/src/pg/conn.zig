...
fn trace(comptime fmt: []const u8, args: anytype) void {
    if (TRACE) std.debug.print("[PG.CONN] " ++ fmt ++ "\n", args);
}
...
pub const Conn = struct {
    ...
    // BackendKeyData: stored for cancel-request support
    _backend_pid: i32 = 0,
    _backend_key: i32 = 0,
    ...
    // Bounded: when cache exceeds _stmt_cache_max, LRU entry is evicted.
    ...
    // Names of prepared statements evicted from local cache that need to be
    // deallocated on the PostgreSQL server. Drained during the next prepare()
    // by prepending Close messages to the PARSE batch — zero extra round-trips.
    _pending_deallocates: std.ArrayListUnmanaged([]const u8),

    // Free-list ownership guard. TRUE exactly when this connection is resident
    // in its pool's free-list (Pool._conns). The pool's stack free-list is only
    // sound under the invariant "each connection is returned at most once per
    // checkout" — but several cleanup paths (the async-cancellation orphan
    // release, the transaction finally-block, thread-owned release at pool
    // close, pinned release) can each call release on the SAME connection under
    // the wrong interleaving. Pool.release atomically claims via this flag, so a
    // duplicate return is a structural no-op; without it a second return places
    // the pointer in two free-list slots and teardown double-frees it. Atomic
    // because those paths run on different threads under the no-GIL runtime.
    _in_pool: std.atomic.Value(bool) = std.atomic.Value(bool).init(false),
    ...
    pub const Opts = struct {
        ...
        connect_timeout_ms: u32 = 0, // 0 = OS default (blocking), >0 = non-blocking + poll
    ...
    pub const QueryOpts = struct {
        ...
        // Optional inferred parameter type OIDs, declared in the Parse
        // message (one per $N placeholder, in order). When null, Parse
        // declares zero param types and PostgreSQL infers them from context
        // (the historical behavior). Supplying concrete OIDs is required for
        // shapes where context inference resolves the WRONG type — most
        // notably `col = ANY(ARRAY[$1,$2,...])`, where the ARRAY constructor
        // resolves unknown params to text BEFORE the outer comparison can
        // propagate the column's type, yielding `integer = text`. A 0 entry
        // means "leave this one to inference" (used for str/None/date/uuid/
        // jsonb, whose text→target coercion must be preserved).
        param_oids: ?[]const i32 = null,
    };
    ...
    pub fn openWithContext(allocator: Allocator, opts: Opts, ssl_ctx: ?*SSLCtx) !Conn {
        ...
        return .{
            ...
            ._stmt_cache_max = 256,
            ._pending_deallocates = .empty,
            ._created_at = @import("pg.zig").timestamp(),
        };
    }
    ...
    pub fn deinit(self: *Conn) void {
        ...
        // Best-effort Terminate message to PostgreSQL.
        // MUST use raw C sendto() because Zig's std.posix.sendmsg declares
        // EPIPE and EBADF as unreachable (panic). During pool teardown,
        // connections may have broken sockets (peer closed, pool exhausted,
        // pinned but unreleased). The Terminate is a courtesy — if it fails,
        // PostgreSQL cleans up on its own via TCP keepalive/timeout.
        {
            const terminate = [_]u8{ 'X', 0, 0, 0, 4 };
            _ = std.c.sendto(self._stream.socket, &terminate, terminate.len, 0, null, 0);
        }
        lib.freeSSLContext(self._ssl_ctx);
        self._stream.close();

        self._stmt_cache_order.deinit(self._allocator);
        self._pending_deallocates.deinit(self._allocator);
    ...
    pub fn age(self: *const Conn) i64 {
        return @import("pg.zig").timestamp() - self._created_at;
    }

    // ── LRU stmt cache helpers ──────────────────────────────────────────

    pub fn stmtCacheMoveToEnd(self: *Conn, name: []const u8) void {
    ...
    /// Atomically insert a prepared-statement describe into both the hashmap
    /// and the LRU order list. If the order-list append fails on OOM, the
    /// hashmap insert is rolled back so the two structures stay consistent.
    /// Caller owns `owned_name` (typically arena-allocated); on success the
    /// hashmap takes ownership.
    pub fn stmtCacheInsert(
        self: *Conn,
        owned_name: []const u8,
        describe: Stmt.Describe,
    ) !void {
        // Pre-reserve the LRU slot first; if this fails we have not yet
        // mutated either container.
        try self._stmt_cache_order.ensureUnusedCapacity(self._allocator, 1);
        try self._prepared_statements.put(self._allocator, owned_name, describe);
        // append is now guaranteed to succeed (capacity reserved above).
        self._stmt_cache_order.appendAssumeCapacity(owned_name);
    }

    pub fn stmtCacheEvictOldest(self: *Conn) void {
        ...
        if (self._prepared_statements.fetchRemove(oldest_name)) |kv| {
            // Queue the name for server-side DEALLOCATE on next prepare().
            // The name string was allocated in the describe arena, so we must
            // dupe it before freeing that arena.
            const duped = self._allocator.dupe(u8, kv.key) catch {
                // OOM — skip deallocate, statement leaks server-side until disconnect.
                var describe = kv.value;
                describe.arena.deinit();
                return;
            };
            self._pending_deallocates.append(self._allocator, duped) catch {};
            var describe = kv.value;
            describe.arena.deinit();
        }
    }

    /// Evict a specific prepared statement by name — targeted surgical
    /// removal for cached plan error recovery. Unlike `stmtCacheEvictOldest`
    /// which removes the LRU entry, this removes exactly the broken entry
    /// so the retry can re-prepare just that one statement.
    ///
    /// The entry is queued for server-side DEALLOCATE on the next prepare()
    /// call (zero extra round-trips, same as LRU eviction). No-op if the
    /// name is not in the cache.
    pub fn stmtCacheEvict(self: *Conn, name: []const u8) void {
        const removed = self._prepared_statements.fetchRemove(name) orelse return;
        // Remove from LRU order list
        ...
        // Queue for server-side DEALLOCATE
        const duped = self._allocator.dupe(u8, removed.key) catch {
            var describe = removed.value;
            describe.arena.deinit();
            return;
        };
        self._pending_deallocates.append(self._allocator, duped) catch {};
    ...
    /// Clear the connection error state. Called before cached-plan retry
    /// so the retry starts with a clean slate. Follows the same pattern
    /// as `release()` which nulls `err` without freeing `_err_data` —
    /// the `_err_data` allocation is reclaimed on next `setErr()` or
    /// `deinit()`.
    pub fn clearErr(self: *Conn) void {
        self.err = null;
    }

    /// Reset session-level state on the connection. Called by pool.release()
    /// before returning the connection to the available list. Clears:
    ///   - SET variables (search_path, statement_timeout, etc.)
    ///   - Open cursors (CLOSE ALL)
    ///   - LISTEN subscriptions (UNLISTEN *)
    ///
    /// Does NOT touch prepared statements — those are managed by the
    /// LRU statement cache and would be wasteful to re-prepare.
    pub fn resetSession(self: *Conn) !void {
        // Reset ALL session-level state on pool release in a SINGLE simple-
        // protocol round trip. The simple Query message accepts multiple
        // `;`-separated statements; PostgreSQL runs them in sequence and
        // replies with one CommandComplete PER statement followed by a single
        // ReadyForQuery. execOpts's read loop already drains every 'C' before
        // the terminating 'Z', so one call collapses what used to be three
        // separate network round-trips (RESET ALL / CLOSE ALL / UNLISTEN *)
        // into one — cutting two round-trips off every pooled release.
        //
        // Uses execOpts (simple protocol), NOT queryOpts — the extended
        // protocol leaves _state in .query until Result.deinit, and since
        // we discard the result, the drain never happens.
        //
        //   RESET ALL  — all SET variables (search_path, statement_timeout, etc.)
        //   CLOSE ALL  — all open cursors (DECLARE CURSOR leftovers)
        //   UNLISTEN * — all LISTEN subscriptions (channel notifications)
        _ = try self.execOpts("RESET ALL; CLOSE ALL; UNLISTEN *", .{}, .{});
    }
    ...
    pub fn auth(self: *Conn, opts: AuthOpts) !void {
        ...
        while (true) {
            ...
            switch (msg.type) {
                'Z' => return,
                'K' => {
                    // BackendKeyData: process ID (4 bytes) + secret key (4 bytes)
                    // Stored for potential cancel-request support
                    if (msg.data.len >= 8) {
                        self._backend_pid = std.mem.readInt(i32, msg.data[0..4], .big);
                        self._backend_key = std.mem.readInt(i32, msg.data[4..8], .big);
                    }
                },
    ...
    /// Execute a query via the extended protocol (Parse/Bind/Execute/Sync).
    ///
    /// **CALLER MUST call result.deinit()** to drain the ReadyForQuery
    /// message and transition the connection back to idle state. Failing
    /// to deinit will leave the connection in `.query` state, causing
    /// subsequent queries to fail with `error.ConnectionBusy`.
    ///
    /// Use `errdefer result.deinit()` or `defer result.deinit()` in all
    /// callers to guarantee cleanup on all code paths.
    pub fn queryOpts(self: *Conn, sql: []const u8, values: anytype, opts: QueryOpts) !*Result {
        // Clear previous error at start of each query
        self.err = null;

        if (self.canQuery() == false) {
            trace("queryOpts: ConnectionBusy state={s} sql={s}", .{ @tagName(self._state), sql[0..@min(sql.len, 60)] });
            self.maybeRelease(opts.release_conn);
            return error.ConnectionBusy;
        }
        self._query_count += 1;

        // F5: single release owner for every error exit below. The prepare,
        // cache-hit, bind, and param-count paths used to `return err` while
        // only `stmt.deinit()`-ing — leaking the acquired connection back to
        // nowhere (pool exhaustion). One errdefer releases on ALL error exits;
        // the success path returns the Result, which owns release_conn.
        ...
        if (name) |n| {
            if (self._prepared_statements.getPtr(n)) |describe| {
                cached = true;
                trace("queryOpts: CACHE HIT name={s} conn={x} sql={s}", .{ n, @intFromPtr(self), sql[0..@min(sql.len, 60)] });
        ...
        if (cached == false) {
            // Check if the last ReadyForQuery said we're in a transaction
            trace("queryOpts: CACHE MISS name={s} conn={x} state={s} qcount={d} stmt_cache_size={d} sql={s}", .{
                if (name) |n| n else "(none)",
                @intFromPtr(self),
                @tagName(self._state),
                self._query_count,
                self._prepared_statements.count(),
        ...
        trace("queryOpts: prepare done, binding {d} params conn={x} sql={s}", .{
            @as(u32, values.len), @intFromPtr(self), sql[0..@min(sql.len, 60)],
        });
        ...
        trace("queryOpts: executing conn={x} sql={s}", .{ @intFromPtr(self), sql[0..@min(sql.len, 60)] });
        return stmt.execute() catch |err| {
            trace("queryOpts: EXECUTE FAILED err={s} conn={x} state={s} sql={s}", .{
                @errorName(err),
                @intFromPtr(self),
                @tagName(self._state),
                sql[0..@min(sql.len, 80)],
            ...
            if (self.err) |pg_err| {
                trace("queryOpts: PG ERROR: {s}", .{pg_err.message[0..@min(pg_err.message.len, 200)]});
            }
            stmt.deinit();
            // conn release handled by the top-level errdefer (F5)
            return err;
        };
    }
    ...
    pub fn execOpts(self: *Conn, sql: []const u8, values: anytype, opts: QueryOpts) !?i64 {
        ...
        if (values.len == 0) {
            ...
            // OPTIMIZATION NOTE: For parameterized execute (non-SELECT), we use the
            // full queryOpts path which includes a Describe phase. The Describe is
            // needed on first execution to learn parameter types, but is already
            // skipped for cached statements (via Stmt.fromDescribe). The uncached
            // path could be optimized to skip Describe and send Parse+Bind+Execute+Sync
            // in one round-trip, but this requires knowing parameter OIDs upfront.
            // For now, the prepared statement cache handles the hot path efficiently —
            // use cache_name on frequently-executed statements for best performance.
        ...
        while (true) {
            ...
            switch (msg.type) {
                'C' => {
                    // Fast path: most mutates affect exactly 1 row.
                    // CommandComplete wire format is "TAG N\0" where N
                    // is the affected row count. Check for " 1\0" suffix
                    // to skip the full parse+parseInt for the common case.
                    const d = msg.data;
                    if (d.len >= 3 and d[d.len - 2] == '1' and d[d.len - 3] == ' ') {
                        affected = 1;
                    } else {
                        const cc = try proto.CommandComplete.parse(d);
    ...
    pub fn deallocate(self: *Conn, cache_name: []const u8) !void {
        if (self._prepared_statements.fetchRemove(cache_name)) |kv| {
            // Also drop the name from the LRU order list. The name is owned by the
            // arena we're about to deinit, so leaving it in _stmt_cache_order would
            // make the next stmtCacheMoveToEnd/Evict run mem.eql on freed memory
            // (UAF) — mirror stmtCacheEvict's orderedRemove.
            for (self._stmt_cache_order.items, 0..) |item, i| {
    ...
    pub fn read(self: *Conn) !lib.Message {
        ...
        while (true) {
            ...
            switch (msg.type) {
                'Z' => {
                    // ReadyForQuery carries a 1-byte transaction-status
                    // indicator. The length is server-controlled: a desynced or
                    // hostile peer can send a zero-length 'Z' body, so guard
                    // before indexing data[0] (UB under ReleaseFast otherwise).
                    if (msg.data.len < 1) return self.unexpectedDBMessage();
                    self._state = switch (msg.data[0]) {
                        'I' => .idle,
                        'T' => .transaction,
                        'E' => blk: {
                            // Failed transaction state — need ROLLBACK to recover.
                            // Without this, the connection is permanently poisoned.
                            trace("read: ReadyForQuery state=E (failed txn), sending ROLLBACK to recover", .{});
                            self._state = .fail;
                            // Send ROLLBACK to clear the failed transaction
                            var buf = &self._buf;
                            buf.reset();
                            const rollback_q = proto.Query{ .sql = "ROLLBACK" };
                            rollback_q.write(buf) catch break :blk .fail;
                            self.write(buf.string()) catch break :blk .fail;
                            // Read ROLLBACK response: CommandComplete + ReadyForQuery
                            while (true) {
                                const rmsg = reader.next() catch break :blk .fail;
                                if (rmsg.type == 'Z') {
                                    if (rmsg.data.len < 1) break :blk .fail;
                                    break :blk switch (rmsg.data[0]) {
                                        'I' => .idle,
                                        else => .fail,
                                    };
                                }
                        ...
                        // Any other status byte is an out-of-spec value from a
                        // desynced/hostile peer. `unreachable` was UB under
                        // ReleaseFast (and a panic under ReleaseSafe) on
                        // server-controlled data — fail the connection instead.
                        else => return self.unexpectedDBMessage(),
                    };
                    return msg;
                },
                'S' => {}, // ParameterStatus: server config params (encoding, timezone, etc.) — informational, no action needed
                'N' => {}, // NoticeResponse: warnings/info from server — informational, no action needed
    ...
    fn setErr(self: *Conn, data: []const u8) error{ PG, OutOfMemory } {
        ...
        if (TRACE) {
            if (self.err) |e| {
                std.debug.print("[PG.CONN] setErr: severity={s} code={s} msg={s}\n", .{
                    e.severity, e.code, e.message[0..@min(e.message.len, 150)],
                });
            }
        }
        return error.PG;
    ...
    fn canQuery(self: *Conn) bool {
        const state = self._state;
        if (state == .idle or state == .transaction) {
            return true;
        }
        if (state == .fail) {
            // Attempt recovery: send ROLLBACK to clear any failed transaction
            trace("canQuery: state=fail, attempting ROLLBACK recovery", .{});
            self.rollback() catch {
                trace("canQuery: ROLLBACK failed, connection unrecoverable", .{});
                return false;
            };
            trace("canQuery: recovered to state={s}", .{@tagName(self._state)});
    ...
    // ── Pipeline: batch N queries in a single round-trip ─────────────────
    //
    // Uses the simple query protocol: writes N 'Q' messages to the buffer,
    // sends them all in one write(), then reads N result sets.
    // Each query gets its own Result with rows + column metadata.
    //
    // Performance: eliminates (N-1) network round-trips for N independent queries.

    pub const PipelineQuery = struct {
        sql: []const u8,
    };

    pub const PipelineResults = struct {
        results: []PipelineResultEntry,
        allocator: Allocator,

        pub const PipelineResultEntry = struct {
            column_names: [][]const u8,
            column_oids: []i32,
            rows: [][]Result.State.Value,
            err: ?[]const u8,
        };

        pub fn deinit(self: *PipelineResults) void {
            for (self.results) |*entry| {
                for (entry.column_names) |name| self.allocator.free(name);
                self.allocator.free(entry.column_names);
                self.allocator.free(entry.column_oids);
                for (entry.rows) |rv| self.allocator.free(rv);
                self.allocator.free(entry.rows);
                if (entry.err) |e| self.allocator.free(e);
            }
            self.allocator.free(self.results);
        }
    };

    pub fn pipeline(self: *Conn, queries: []const PipelineQuery) !*PipelineResults {
        if (!self.canQuery()) return error.ConnectionBusy;
        if (queries.len == 0) {
            const results = try self._allocator.create(PipelineResults);
            results.* = .{ .results = &.{}, .allocator = self._allocator };
            return results;
        }

        self._query_count += @intCast(queries.len);

        // 1. Write all Query messages to buffer
        self._buf.reset();
        for (queries) |q| {
            const query_msg = proto.Query{ .sql = q.sql };
            try query_msg.write(&self._buf);
        }

        // 2. Single write to socket
        try self.write(self._buf.string());
        ...
        const entries = try alloc.alloc(PipelineResults.PipelineResultEntry, queries.len);
        for (entries) |*e| {
            e.column_names = &.{};
            e.column_oids = &.{};
            e.rows = &.{};
        ...
        // F9: without this, any error return mid-read (a fatal I/O/protocol
        // error, or an OOM while duping a row) leaked `entries` plus every
        // already-filled entry's column/row allocations. Frees exactly what
        // PipelineResults.deinit frees, so cleanup is identical on either path.
        errdefer {
            for (entries) |*entry| {
                for (entry.column_names) |name| alloc.free(name);
                alloc.free(entry.column_names);
                alloc.free(entry.column_oids);
                for (entry.rows) |rv| alloc.free(rv);
                alloc.free(entry.rows);
                if (entry.err) |e| alloc.free(e);
            }
            alloc.free(entries);
        }

        // 5. Read results for each query
        var query_idx: usize = 0;
        while (query_idx < queries.len) {
            var col_names: std.ArrayListUnmanaged([]const u8) = .empty;
            var col_oids: std.ArrayListUnmanaged(i32) = .empty;
            var rows: std.ArrayListUnmanaged([]Result.State.Value) = .empty;
            // F9: these in-progress lists aren't owned by `entries` until the
            // toOwnedSlice calls below, so they need their own errdefers or
            // they leak on a mid-query error. After toOwnedSlice the lists are
            // empty, so these become no-ops (ownership moved into `entries`,
            // freed by the entries errdefer) — no double free.
            errdefer {
                for (col_names.items) |name| alloc.free(name);
                col_names.deinit(alloc);
            }
            errdefer col_oids.deinit(alloc);
            errdefer {
                for (rows.items) |rv| alloc.free(rv);
                rows.deinit(alloc);
            }

            // Read messages until CommandComplete for this query
            while (true) {
                const msg = self.read() catch |err| {
                    // F9: read() converts a server ErrorResponse ('E') into
                    // error.PG *before* the switch below ever sees it — so the
                    // old 'E' case was dead and a failing query used to throw
                    // out of pipeline(), leaving the trailing ReadyForQuery
                    // unread and _state=.idle (a desynced conn later reused).
                    // Per-query server errors are recorded and recovered from
                    // (the trailing 'Z' is consumed below); anything else is a
                    // fatal desync, so mark the conn failed so the pool
                    // replaces it instead of silently reusing it.
                    if (err == error.PG) {
                        if (self.err) |pg_err| {
                            entries[query_idx].err = alloc.dupe(u8, pg_err.message) catch null;
                        }
                        break;
                    }
                    self._state = .fail;
                ...
                switch (msg.type) {
                    'T' => {
                        ...
                        const data = msg.data;
                        // 2-byte column count is server-controlled: guard before
                        // reading it (a truncated 'T' body would OOB otherwise).
                        if (data.len < 2) return self.unexpectedDBMessage();
                        const ncols = std.mem.readInt(u16, data[0..2], .big);
                        var offset: usize = 2;
                        for (0..ncols) |_| {
                            // Column name (null-terminated)
                            const name_end = std.mem.indexOfPos(u8, data, offset, &.{0}) orelse break;
                            const name = try alloc.dupe(u8, data[offset..name_end]);
                            try col_names.append(alloc, name);
                            offset = name_end + 1;
                            // table_oid(4) + col_num(2) + type_oid(4) + type_size(2) + type_mod(4) + format(2) = 18
                            if (offset + 18 <= data.len) {
                                const oid = std.mem.readInt(i32, data[offset + 6 ..][0..4], .big);
                                try col_oids.append(alloc, oid);
                                offset += 18;
                            }
                        }
                    },
                    'D' => {
                        // DataRow — parse row values. Every per-column length is
                        // attacker-controlled wire data; parseDataRow bounds-checks
                        // each one (see result.zig:_next for the mirror guard).
                        const row_vals = parseDataRow(alloc, msg.data) catch |err| {
                            self._state = .fail;
                            return err;
                        };
                        try rows.append(alloc, row_vals);
                    },
                    'C' => {
                        // CommandComplete — this query is done
                        break;
                    },
                    // 'E' (ErrorResponse) is unreachable here: self.read()
                    // intercepts it and returns error.PG, handled in the catch
                    // above (F9).
                    'Z' => {
                        // ReadyForQuery — shouldn't happen mid-pipeline in simple protocol
                        // but handle gracefully
                        break;
                    },
                    else => {
                        // Skip unknown messages (NoticeResponse, etc.)
                    },
                }
            }

            entries[query_idx].column_names = try col_names.toOwnedSlice(alloc);
            entries[query_idx].column_oids = try col_oids.toOwnedSlice(alloc);
            entries[query_idx].rows = try rows.toOwnedSlice(alloc);

            // Consume ReadyForQuery after each query in simple protocol
            const rfq = try self.read();
            if (rfq.type != 'Z') {
                return self.unexpectedDBMessage();
            }
        ...
        const results = try alloc.create(PipelineResults);
        results.* = .{ .results = entries, .allocator = alloc };
        return results;
    }
};

/// Parse a DataRow ('D') message body (`data` begins at the 2-byte column
/// count) into owned column values. Every length prefix here is attacker-
/// controlled wire data, so this mirrors result.zig:_next: guard the count
/// read, guard `offset+4 <= data.len` before each length, reject `length < -1`
/// before `@intCast`, and confirm the sliced payload stays in bounds. On any
/// error the already-duped payloads and the row slice are freed (no leak).
fn parseDataRow(alloc: Allocator, data: []const u8) ![]Result.State.Value {
    if (data.len < 2) return error.InvalidDataRow;
    const ncols = std.mem.readInt(u16, data[0..2], .big);
    const row_vals = try alloc.alloc(Result.State.Value, ncols);
    var filled: usize = 0;
    errdefer {
        for (row_vals[0..filled]) |v| if (!v.is_null) alloc.free(v.data);
        alloc.free(row_vals);
    }
    var offset: usize = 2;
    for (0..ncols) |ci| {
        ...
        if (length == -1) {
            ...
            row_vals[ci] = .{ .is_null = false, .data = try alloc.dupe(u8, data[data_start..data_end]) };
            offset = data_end;
        }
        filled = ci + 1;
    }
...
test "parseDataRow: valid row with data + null columns" {
    // 2 cols: col0 = "hi" (len 2), col1 = NULL (len -1)
    const frame = [_]u8{
        0, 2, // column count = 2
        0, 0, 0, 2, 'h', 'i', // col0: length 2, "hi"
        0xFF, 0xFF, 0xFF, 0xFF, // col1: length -1 (NULL)
    };
    const row = try parseDataRow(t.allocator, &frame);
    defer {
        for (row) |v| if (!v.is_null) t.allocator.free(v.data);
        t.allocator.free(row);
    }
    try t.expectEqual(@as(usize, 2), row.len);
    try t.expectEqual(false, row[0].is_null);
    try t.expectString("hi", row[0].data);
    try t.expectEqual(true, row[1].is_null);
}

test "parseDataRow: truncated count is rejected" {
    try t.expectError(error.InvalidDataRow, parseDataRow(t.allocator, &[_]u8{0}));
    try t.expectError(error.InvalidDataRow, parseDataRow(t.allocator, &[_]u8{}));
}

test "parseDataRow: missing length bytes is rejected" {
    // count says 1 column but no 4-byte length follows
    try t.expectError(error.InvalidDataRow, parseDataRow(t.allocator, &[_]u8{ 0, 1, 0, 0 }));
}

test "parseDataRow: length overflowing the frame is rejected" {
    // count=1, length=100 but only 1 payload byte present
    const frame = [_]u8{ 0, 1, 0, 0, 0, 100, 'x' };
    try t.expectError(error.InvalidDataRow, parseDataRow(t.allocator, &frame));
}

test "parseDataRow: negative length below -1 is rejected" {
    // count=1, length=-2 (0xFFFFFFFE) — must reject before @intCast
    const frame = [_]u8{ 0, 1, 0xFF, 0xFF, 0xFF, 0xFE };
    try t.expectError(error.InvalidDataRow, parseDataRow(t.allocator, &frame));
}

test "Conn: auth trust (no pass)" {
    try t.requireFixture();
...
test "PG: type support" {
    ...
    {
        // charn, charn[] — col_charn is char(3). The driver preserves bpchar's
        // blank padding (faithful to what PG stores; it does not silently strip
        // trailing spaces): "Teg" fills the width exactly, while the 2-char array
        // elements come back padded to 3 ("NR ", "hi ").
...
test "PG: query column names" {
    ...
    defer c.deinit();
    {
        // hyperdjango defaults column_names to true (the native JSON path needs
        // them); request it off explicitly to cover the empty case.
        var result = try c.queryOpts("select 1 as id, 'leto' as name", .{}, .{ .column_names = false });
...
test "PG: cached query" {
    ...
    {
        var result = try c.queryOpts("select $1::int as id, $2::text as name", .{ 1, "leto" }, .{ .cache_name = "c1", .column_names = false });
        try t.expectEqual(0, result.column_names.len);
        const row = (try result.nextUnsafe()) orelse unreachable;
        try t.expectEqual(1, row.get(i32, 0));
    ...
    {
        var result = try c.queryOpts("slc", .{ 2, "ghanima" }, .{ .cache_name = "c1", .column_names = false });
        try t.expectEqual(0, result.column_names.len);
        const row = (try result.nextUnsafe()) orelse unreachable;
