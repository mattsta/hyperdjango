...
/// Set SO_RCVTIMEO via libc directly, NOT std.posix.setsockopt. That wrapper
/// asserts `unreachable` on EBADF/ENOTSOCK/EINVAL — exactly the errnos a
/// peer-closed connection socket returns (a cancelled or torn-down pinned
/// transaction leaves such a socket), and the caller's `try` cannot catch a
/// panic raised INSIDE the wrapper: it aborts the worker thread instead. Turn a
/// failure into a returnable error so the caller unwinds the flow cleanly.
fn setRcvTimeout(socket: posix.socket_t, tv: []const u8) error{SetSockOpt}!void {
    const rc = std.c.setsockopt(
        socket,
        @as(c_int, posix.SOL.SOCKET),
        @as(c_int, posix.SO.RCVTIMEO),
        tv.ptr,
        @intCast(tv.len),
    );
...
// Upper bound on a single PG wire message. The 4-byte length prefix is an Int32
// and legitimate messages never approach this; capping here turns a corrupt/
// hostile length into a clean protocol error instead of a multi-GB allocation.
const max_message_length: usize = 1 << 30; // 1 GiB
    ...
    return struct {
        ...
        pub fn startFlow(self: *Self, allocator: ?Allocator, timeout_ms: ?u32) !void {
            if (timeout_ms) |ms| {
                ...
                try setRcvTimeout(self.stream.socket, &timeval);
                self.has_timeout = true;
            } else if (self.has_timeout) {
                try setRcvTimeout(self.stream.socket, &zero_timeval);
        ...
        fn read(self: *Self, error_peek: bool) !Message {
            ...
            while (true) {
                if (message_length == 0) {
                    ...
                    if (current_length > 4) {
                        // The 4-byte length prefix includes itself, so the
                        // minimum legal value is 4. Validate in usize before any
                        // arithmetic so a bogus length can't underflow/overflow.
                        const wire_len: usize = std.mem.readInt(u32, buf[start + 1 .. start + 5][0..4], .big);
                        if (wire_len < 4 or wire_len > max_message_length) {
                            return error.InvalidMessageLength;
                        }
                        // + 1 for the type byte (usize math — no u32 wrap)
                        message_length = wire_len + 1;
        ...
        fn buffered(self: *Self, pos: usize, error_peek: bool) ?Message {
            ...
            // Read the wire length into a usize. The 4-byte prefix includes
            // itself, so the minimum legal value is 4. A value < 4 would make
            // `end` (len_end + len - 4) underflow into a giant slice, and a value
            // near u32-max must not wrap the `available < len + 1` guard. On a
            // violation we bail to the slow path (read), which raises a clean
            // protocol error rather than slicing out of bounds.
            const len: usize = std.mem.readInt(u32, buf[start + 1 .. len_end][0..4], .big);
            if (len < 4 or len > max_message_length) {
                return null;
            }
...
test "Reader: rejects invalid message length" {
    const R = ReaderT(*t.Stream);
    var s = t.Stream.init();
    defer s.deinit();

    {
        // len prefix of 2 is illegal (< 4) — must not underflow the slice math
        s.reset();
        s.add(&[_]u8{ 1, 0, 0, 0, 2, 9 });
        var reader = R.init(t.allocator, 16, s) catch unreachable;
        defer reader.deinit();
        try t.expectError(error.InvalidMessageLength, reader.next());
    }

    {
        // len prefix of 0xFFFFFFFF must not wrap the `available < len + 1` guard
        s.reset();
        s.add(&[_]u8{ 1, 255, 255, 255, 255, 9 });
        var reader = R.init(t.allocator, 16, s) catch unreachable;
        defer reader.deinit();
        try t.expectError(error.InvalidMessageLength, reader.next());
    }
