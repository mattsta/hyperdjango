...
pub fn parse(data: []const u8) Error {
    ...
    while (pos < data.len) {
        ...
        switch (data[pos]) {
            ...
            'L' => err.line = value,
            'R' => err.routine = value,
            // Per the PostgreSQL protocol, frontends MUST silently ignore
            // unrecognized ErrorResponse/NoticeResponse field types. `value`
            // has already been delimited, so `pos` still advances past the
            // whole field below and parsing continues.
            else => {},
        }
        pos = value_end + 1;
    }
...
test "Error: parse" {
    ...
    {
        // V3: an unrecognized field type must be silently skipped (not hit
        // `unreachable`), and parsing must continue past it to later fields.
        buf.reset();
        ...
        // Unknown field byte 'Z' (not in the recognized set) with a value.
        try buf.writeByte('Z');
        try buf.write("ignored-unknown");
        try buf.writeByte(0);

        try buf.writeByte('M');
        try buf.write("after-unknown");
        try buf.writeByte(0);

        const err = Error.parse(buf.string());
        try t.expectString("22001", err.code);
        try t.expectString("after-unknown", err.message);
    }
}
