...
pub const Char = struct {
    ...
    pub fn decodeKnown(data: []const u8) u8 {
        // Wire field may be shorter than the type width (truncated/desynced row).
        // Guard before the read so the row.get / iterator / record paths return a
        // sentinel instead of over-reading past the slice.
...
pub const Timestamp = struct {
    ...
    pub fn decodeKnown(data: []const u8) i64 {
        // Guard the fixed 8-byte read; a truncated field yields the epoch base
        // (payload-zero) sentinel rather than over-reading.
        if (data.len < 8) return us_from_epoch_to_y2k;
...
// ── JSON coercion for text params bound to a JSON/JSONB column ──────────────
//
// db.zig's extractParams serializes a Python `dict` via json.dumps (valid JSON) but a
// Python `list`/`tuple` as a PostgreSQL array literal `{a,b}` (pyListToPgArray) — which is
// NOT valid JSON and is rejected by a JSONB column ("invalid input syntax for type json").
// The destination OID is only known here at bind time, so this is the correct place to
// repair it: when a `[]const u8` text param targets JSON (114) / JSONB (3802) and the text
// is a PG array literal, rewrite it to a JSON array. Already-valid JSON (a dict's
// `{"k":v}`, a `[..]`, a scalar/string) is passed through byte-for-byte unchanged.

// Decide whether `v` is a PG array literal that must be rewritten to JSON.
// True only for `{...}` whose top level is array-shaped (no top-level key `:` after the
// first element) — distinguishing a PG string array `{"a","b"}` from a JSON object
// `{"a":1}`. A `[...]`, a JSON object, or any scalar returns false (pass through as-is).
fn isPgArrayLiteral(v: []const u8) bool {
    if (v.len < 2 or v[0] != '{') return false;
    // `{}` is AMBIGUOUS — both an empty PG array literal and an empty JSON object. Default to JSON
    // object (NOT a PG array), i.e. pass it through unchanged: a `dict` / `json.dumps({})` → `{}` is by
    // far the common case and is already valid JSON, so coercing it to `[]` (the old bug) corrupted every
    // empty-object bound to JSONB. The trade: a RAW empty `list` bound to JSONB also stores `{}` (an empty
    // object); a caller needing an empty JSON ARRAY should bind `[]` / `json.dumps([])` (MESH's
    // JSONBListModel pre-encodes its list columns, so it is unaffected). A real EMPTY PG-array bind targets
    // an array-OID column, which never reaches this JSONB/JSON coercion path. Non-empty array literals
    // (`{a,b}`, `{a}`) are still detected + coerced below.
    if (v.len == 2 and v[1] == '}') return false;
    // Scan depth-1 tokens; if we hit a top-level ':' before a top-level ',' or the close,
    // it is a JSON object (key:value), not a PG array.
    var i: usize = 1;
    var in_str = false;
    while (i < v.len) : (i += 1) {
        const ch = v[i];
        ...
        switch (ch) {
            '"' => in_str = true,
            ':' => return false, // top-level key:value → JSON object
            ',', '}' => return true, // first top-level separator (after content) with no ':' → PG array
            else => {},
        }
    }
    return true; // braces with content + no top-level ':' → a PG array literal
}

// Stream a PG array literal `{...}` into `buf` as JSON `[...]`. Closed grammar produced by
// pyListToPgArray: braces, comma-separated elements, double-quoted strings (\\ and \" the
// same escapes JSON uses), nested `{...}`, bare `NULL`, and bare scalars (numbers/bools
// from pyObjToText are already JSON tokens). A bare token that is not a JSON literal
// (number/true/false/null) is quoted so the result is always valid JSON.
fn writePgArrayAsJson(v: []const u8, buf: *buffer.Buffer) !void {
    var i: usize = 0;
    while (i < v.len) : (i += 1) {
        const ch = v[i];
        switch (ch) {
            ...
            '}' => try buf.writeByte(']'),
            '"' => {
                // Copy a quoted string verbatim (PG and JSON share \" and \\ escaping).
                try buf.writeByte('"');
                i += 1;
                while (i < v.len) : (i += 1) {
                    const c2 = v[i];
                    try buf.writeByte(c2);
                    if (c2 == '\\' and i + 1 < v.len) {
                        i += 1;
                        try buf.writeByte(v[i]);
                    } else if (c2 == '"') {
                        break;
                    }
                }
            ...
            ',' => try buf.writeByte(','),
                ...
                // A bare, unquoted top-level token up to the next , } — NULL/number/bool,
                // or (rarely) a str()-fallback value. Quote anything that is not a JSON
                // literal so the output is always valid JSON.
                const start = i;
                while (i < v.len and v[i] != ',' and v[i] != '}' and v[i] != '{') : (i += 1) {}
                const token = std.mem.trim(u8, v[start..i], " ");
                i -= 1; // re-process the terminator on the next loop iteration
                if (token.len == 0) {
                    // nothing
                } else if (std.mem.eql(u8, token, "NULL") or std.mem.eql(u8, token, "null")) {
                    try buf.write("null");
                } else if (isJsonBareLiteral(token)) {
                    try buf.write(token);
                } else {
                    try buf.writeByte('"');
                    try buf.write(token);
...
// Stream a JSON array `[...]` into `buf` as a PG array literal `{...}` — the INVERSE of
// writePgArrayAsJson. Used to bind a Python list that extractParams canonicalised to JSON (an EMPTY
// list `[]`, or a flat list serialized as JSON) into a native scalar-array column, which parses PG-array
// text `{...}`. A flat JSON array's elements (numbers, `true`/`false`/`null`, double-quoted strings —
// PG and JSON share the `\"`/`\\` escaping) are already valid PG-array element tokens, so only the
// surrounding brackets are swapped; quoted strings are copied verbatim so a `,`/`]`/`[` inside a string
// is not mistaken for structure. (`[]`→`{}`, `[1,2]`→`{1,2}`, `["a,b"]`→`{"a,b"}`.)
fn writeJsonAsPgArray(v: []const u8, buf: *buffer.Buffer) !void {
    var i: usize = 0;
    while (i < v.len) : (i += 1) {
        const ch = v[i];
        switch (ch) {
            ...
            else => try buf.writeByte(ch), // commas / numbers / true|false|null / spaces — PG-valid verbatim
        }
    }
}

// Bind a `[]const u8` text param to a native scalar-ARRAY column (int[]/text[]/bool[]/float[]/…). A
// non-empty flat Python list arrives as a PG array literal `{a,b}` (pyListToPgArray) → passes through
// verbatim; an EMPTY list (and any list extractParams canonicalised to JSON) arrives as `[...]` →
// converted to `{...}`. Text format, so Postgres applies its own element-type parse. This closes the
// empty-list→array vs empty-list/dict→JSONB ambiguity at origin: the canonical param form for a list is
// JSON, and each OID branch (JSONB coerces PG-array→JSON; array coerces JSON→PG-array) maps it correctly.
fn encodeTextArrayCoerced(value: []const u8, buf: *buffer.Buffer, format_pos: usize) !void {
    if (value.len == 0 or value[0] != '[') return String.encode(value, buf, format_pos);
    buf.writeAt(&text_encoding, format_pos);
    const state = try Encode.variableLengthStart(buf);
    try writeJsonAsPgArray(value, buf);
    Encode.variableLengthFill(buf, state);
}

// True if `tok` is a JSON-safe bare literal: true / false, or a JSON number.
fn isJsonBareLiteral(tok: []const u8) bool {
    if (std.mem.eql(u8, tok, "true") or std.mem.eql(u8, tok, "false")) return true;
    var seen_digit = false;
    for (tok, 0..) |ch, idx| {
        switch (ch) {
            '0'...'9' => seen_digit = true,
            '-', '+' => if (idx != 0 and tok[idx - 1] != 'e' and tok[idx - 1] != 'E') return false,
            '.', 'e', 'E' => {},
            else => return false,
        }
    }
...
pub const JSON = struct {
    ...
    // Bind a text param to a JSON column, coercing a PG array literal to a JSON array.
    fn encodeBytesCoerced(value: []const u8, buf: *buffer.Buffer, format_pos: usize) !void {
        if (!isPgArrayLiteral(value)) return JSON.encodeBytes(value, buf, format_pos);
        buf.writeAt(JSON.encoding, format_pos);
        const state = try Encode.variableLengthStart(buf);
        try writePgArrayAsJson(value, buf);
        Encode.variableLengthFill(buf, state);
    }
...
pub const JSONB = struct {
    ...
    // Bind a text param to a JSONB column, coercing a PG array literal `{a,b}` into a JSON
    // array `[a,b]` first. A Python dict (already valid JSON) and an already-`[...]` value
    // pass straight through encodeBytes — byte-for-byte unchanged.
    fn encodeBytesCoerced(value: []const u8, buf: *buffer.Buffer, format_pos: usize) !void {
        if (!isPgArrayLiteral(value)) return JSONB.encodeBytes(value, buf, format_pos);
        buf.writeAt(JSONB.encoding, format_pos);
        const state = try Encode.variableLengthStart(buf);
...
fn bindSlice(oid: i32, value: anytype, buf: *buffer.Buffer, format_pos: usize) !void {
    ...
    if (T == []u8 or T == []const u8) {
        switch (oid) {
            ...
            // Coerce a PG array literal (from a Python list) into a JSON array; valid JSON
            // (a dict, an already-`[...]`) passes through unchanged. Fixes list→JSONB bind.
            JSONB.oid.decimal => return JSONB.encodeBytesCoerced(value, buf, format_pos),
            JSON.oid.decimal => return JSON.encodeBytesCoerced(value, buf, format_pos),
            // Native scalar/text ARRAY columns (bool/int2/int4/text/bpchar/varchar/int8/float4/float8/
            // numeric/timestamp/timestamptz []): a `[]const u8` array param arrives either as PG-array
            // text `{a,b}` (a non-empty flat list, verbatim) or as canonical JSON `[...]` (an EMPTY list,
            // `[]`); coerce the JSON form to PG-array text so an empty list binds as `{}`, not the
            // malformed `[]`. Fixes empty-list→array while empty-list→JSONB stays `[]` (the dual coerce).
            1000, 1005, 1007, 1009, 1015, 1016, 1021, 1022, 1231, 1115, 1185 => return encodeTextArrayCoerced(value, buf, format_pos),
...
// Wave1-A residual: fixed-width decodeKnown must not over-read a wire field that
// is shorter than the type width. On a too-short slice each returns a sentinel;
// on a full-width slice it decodes normally. (In ReleaseSafe an over-read would
// panic, so these truncated cases are the regression guard.)
test "decodeKnown: truncated fixed-width buffers return sentinel (no OOB)" {
    // too-short → sentinel, never touches out-of-range bytes
    try std.testing.expectEqual(@as(u8, 0), Char.decodeKnown(&.{}));
    try std.testing.expectEqual(@as(i16, 0), Int16.decodeKnown(&.{}));
    try std.testing.expectEqual(@as(i16, 0), Int16.decodeKnown(&.{0x01}));
    try std.testing.expectEqual(@as(i32, 0), Int32.decodeKnown(&.{ 0x01, 0x02, 0x03 }));
    try std.testing.expectEqual(@as(i64, 0), Int64.decodeKnown(&.{ 1, 2, 3, 4, 5, 6, 7 }));
    try std.testing.expectEqual(@as(f32, 0), Float32.decodeKnown(&.{ 1, 2, 3 }));
    try std.testing.expectEqual(@as(f64, 0), Float64.decodeKnown(&.{1}));
    try std.testing.expectEqual(false, Bool.decodeKnown(&.{}));
    try std.testing.expectEqual(@as(i64, Timestamp.us_from_epoch_to_y2k), Timestamp.decodeKnown(&.{ 1, 2, 3 }));

    // full-width → normal decode still works
    try std.testing.expectEqual(@as(u8, 0x7a), Char.decodeKnown(&.{0x7a}));
    try std.testing.expectEqual(@as(i16, 0x0102), Int16.decodeKnown(&.{ 0x01, 0x02 }));
    try std.testing.expectEqual(@as(i32, 0x01020304), Int32.decodeKnown(&.{ 0x01, 0x02, 0x03, 0x04 }));
    try std.testing.expectEqual(@as(i64, 0x0102030405060708), Int64.decodeKnown(&.{ 1, 2, 3, 4, 5, 6, 7, 8 }));
    try std.testing.expectEqual(true, Bool.decodeKnown(&.{1}));
    try std.testing.expectEqual(false, Bool.decodeKnown(&.{0}));
}
