...
pub const Numeric = struct {
    ...
    fn encodeValue(value: anytype, buf: *buffer.Buffer) !void {
        ...
        const written = std.fmt.bufPrint(&str_buf, "{d}", .{value}) catch return error.EncodingFailed;
        return encodeValidString(written, buf);
    }
    ...
    pub fn decode(comptime fail_mode: lib.FailMode, data: []const u8, data_oid: i32) if (fail_mode == .unsafe) Numeric else lib.TypeError!Numeric {
        ...
        // The 8-byte header and the number_of_digits it declares are peer-
        // controlled. In .safe mode surface a real error when the payload can't
        // hold the header or the declared base-10000 digits (lib.assert was a
        // no-op in ReleaseFast). In .unsafe mode there is no error channel, so
        // decodeKnown itself is defensive (clamps below).
        if (comptime fail_mode == .safe) {
            if (data.len < 8) return error.InvalidData;
            const nd = std.mem.readInt(u16, data[0..2], .big);
            if (8 + @as(usize, nd) * 2 > data.len) return error.InvalidData;
        }
        return decodeKnown(data);
    }

    pub fn decodeKnown(data: []const u8) Numeric {
        // Defensive against a short/hostile header: without an error channel
        // (decodeKnown feeds the .unsafe path and array iterators) we return a
        // benign zero rather than read OOB.
        if (data.len < 8) {
            return .{ .number_of_digits = 0, .weight = 0, .sign = .positive, .scale = 0, .digits = &[_]u8{} };
        }
        var number_of_digits = std.mem.readInt(u16, data[0..2], .big);
        var digits: []const u8 = data[8..];
        // Clamp the declared digit count to the whole base-10000 digits actually
        // present, and trim `digits` to exactly that many, so toFloat/toString
        // (and any other consumer) can never read past the wire buffer even if
        // number_of_digits was inflated by a malicious peer.
        const available: u16 = @intCast(@min(digits.len / 2, @as(usize, std.math.maxInt(u16))));
        if (number_of_digits > available) number_of_digits = available;
        digits = digits[0 .. @as(usize, number_of_digits) * 2];
        return .{
    ...
    pub fn toFloat(self: Numeric) f64 {
        ...
        for (0..self.number_of_digits) |_| {
            // Bound by the actual slice length: decodeKnown already trims digits
            // to number_of_digits*2, but guard here too so a directly-constructed
            // Numeric can never read past its own digit buffer.
...
// ── Wire-fuzz seeds — numeric decode memory safety ───────────────────────────

test "Numeric.decode: safe mode rejects short header" {
    // < 8 header bytes: lib.assert was a no-op in ReleaseFast; now a real error.
    try std.testing.expectError(error.InvalidData, Numeric.decode(.safe, &[_]u8{ 0, 0, 0 }, 1700));
}

test "Numeric.decode: safe mode rejects inflated number_of_digits" {
    // header declares 100 base-10000 digits but carries none — 8 + 100*2 > 8.
    const data = [_]u8{ 0, 100, 0, 0, 0, 0, 0, 0 };
    try std.testing.expectError(error.InvalidData, Numeric.decode(.safe, &data, 1700));
}

test "Numeric.decodeKnown: short header yields benign zero" {
    const n = Numeric.decodeKnown(&[_]u8{ 1, 2, 3 });
    try std.testing.expectEqual(@as(u16, 0), n.number_of_digits);
    try std.testing.expectEqual(@as(usize, 0), n.digits.len);
    try std.testing.expectEqual(@as(f64, 0), n.toFloat());
}

test "Numeric.decodeKnown: inflated count is clamped, no OOB in toFloat" {
    // number_of_digits=100 but only one real base-10000 digit (0x0003) present.
    const data = [_]u8{ 0, 100, 0, 0, 0, 0, 0, 0, 0, 3 };
    const n = Numeric.decodeKnown(&data);
    // clamped to the single whole digit actually in the buffer
    try std.testing.expectEqual(@as(u16, 1), n.number_of_digits);
    try std.testing.expectEqual(@as(usize, 2), n.digits.len);
    // 3 * 10000^0 == 3, and the loop reads nothing past the 2-byte digit buffer
    try std.testing.expectEqual(@as(f64, 3), n.toFloat());
}

test "Numeric.decodeKnown: valid small value round-trips" {
    // number_of_digits=1, weight=0, sign positive, scale 0, digit group 3
    const data = [_]u8{ 0, 1, 0, 0, 0, 0, 0, 0, 0, 3 };
    try std.testing.expectEqual(@as(f64, 3), Numeric.decodeKnown(&data).toFloat());
}
