...
(function(){
  ...
  function fillSeg(box, items, active, onPick){
    box.innerHTML='';
    items.forEach(it=>{ const b=el('button',{text:it.label}); b.dataset.key=it.key; if(it.key===active)b.classList.add('on'); b.onclick=()=>onPick(it.key); box.appendChild(b); });
  }
  ...
  function syncControls(){
    ...
    E.suiteCtl.classList.toggle('hidden', suiteKeys.length<=1);
    fillSeg(E.suite, suiteKeys.map(k=>({key:k,label:curSuites()[k].label||k})), state.suite, k=>{ state.suite=k; structural(); });
  ...
  function scan(su,sw,gk,agg){ su.variants.forEach(v=>{ ((sw.data[v+'|'+gk]||{})[state.metric]||[]).forEach(x=>{ if(x!=null)agg(x); }); }); }
